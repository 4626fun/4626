...
## Documentation Model (2026-06)

**Public site = curated manual docs only** (~250 pages). Autogenerated TypeDoc / `forge doc` dumps stay in-repo (`pnpm docs:refresh`) and are not published to docs.4626.fun.

| Source | Published? | Location |
|--------|------------|----------|
| **Manual docs** | Yes | `docs/` → site root |
| **Contract / frontend API** | No (local only) | `docs/_generated/` |
| **Internal WIP** | No | `docs/_internal/` |

Production build: `pnpm -C apps/docs-site build:production` (`DOCS_PUBLISH_CURATED=1`).

Full API sync for local dev: `DOCS_PUBLISH_CURATED=0 pnpm -C apps/docs-site prepare:content && pnpm -C apps/docs-site start`.

### Legacy model (pre-2026-06)
