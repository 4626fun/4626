...
const SOURCES = {
  manual: {
    ...
    exclude: [
      ...
      'plans/**',
      'runbooks/**',
      // WIP / engineering-only — keep in repo, not on public site.
      'llms/**',
      'audits/4626/acceptances/**',
      'agent-learned-facts.md',
      // Lanes not in curated sidebar — repo-only or linked via redirects.
      'roadmap/**',
      'product/**',
      'compressions/**',
      'protocols/**',
      ...
      'frontend/index.md',
      'security/amoe-*.md',
      'security/ci-red-baseline-audit.md',
      'security/transactional-integrity-audit.md',
      'security/4626-agent-security-migration.md',
      'audits/codex/**',
      'audits/charm/**',
      'audits/token-image/**',
      'audits/4626/**',
      'audits/x-ray/entry-points.md',
      'audits/x-ray/invariants.md',
      'audits/x-ray/review-todo.md',
      'audits/x-ray/x-ray.md',
      'audits/creatorvault-business-logic-core-structure-audit.md',
    ],
    label: 'Manual docs',
  },
  contracts: {
...
/** Production docs.4626.fun: manual `docs/` only — no typedoc/forge/kpr mirror. */
const CURATED_PUBLISH = process.env.DOCS_PUBLISH_CURATED === '1';
...
async function sync() {
  ...
  // Process each source
  const sourceKeys = CURATED_PUBLISH ? ['manual'] : Object.keys(SOURCES);
  for (const sourceKey of sourceKeys) {
    await processSource(sourceKey, { missingManualAsWarning: preserveBundledSnapshot });
  }

  // API index only when full API docs are synced
  if (!CURATED_PUBLISH) {
    await createApiIndexPages();
  } else {
    console.log('\n📄 Skipped autogenerated API index (curated publish)');
  }
