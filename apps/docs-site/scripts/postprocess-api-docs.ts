...
interface ApiSidebarDomain {
  id: string;
  label: string;
}

interface ApiSidebarRouteMap {
  id: string;
  label: string;
}

interface ApiSidebarManifest {
  domains: ApiSidebarDomain[];
  routeMaps: ApiSidebarRouteMap[];
}
...
async function generateApiSidebarManifest(): Promise<void> {
  if (!(await pathExists(HANDLERS_DIR))) {
    console.log('   ⚠ Skipping API sidebar manifest (handlers dir missing)');
    return;
  }

  const entries = await fs.readdir(HANDLERS_DIR, {withFileTypes: true});
  const domains: ApiSidebarDomain[] = entries
    .filter((entry) => entry.isDirectory())
    .map((entry) => ({id: entry.name, label: toTitleCase(entry.name)}))
    .sort((a, b) => a.label.localeCompare(b.label));

  const routeMaps: ApiSidebarRouteMap[] = entries
    .filter((entry) => entry.isFile() && entry.name.endsWith('.md') && entry.name !== 'index.md')
    .map((entry) => {
      const id = entry.name.replace(/\.md$/, '');
      return {id, label: formatRouteMapLabel(id)};
    })
    .sort((a, b) => a.label.localeCompare(b.label));

  const manifest: ApiSidebarManifest = {domains, routeMaps};
  ...
  const hubLines = [
    ...
    '# HTTP Handlers',
    '',
    'Curated index of Vercel API route handlers under `frontend/api/_handlers/`.',
    'Use search (`Cmd/Ctrl+K`) to jump directly to a symbol.',
    '',
    '## Domains',
    '',
    ...
    ...routeMaps.map(
      (route) =>
        `| ${route.label} | [${route.id}](/api/frontend/api/_handlers/${route.id}) |`,
    ),
    '',
    '---',
    '',
    '*This page is auto-generated by `pnpm api:postprocess`. Do not edit directly.*',
    '',
  ];
...
async function postprocess(): Promise<void> {
  ...
  // Step 3b: Generate API sidebar manifest + handlers hub
  console.log('\n🗂️  Generating API sidebar manifest...');
  await generateApiSidebarManifest();
