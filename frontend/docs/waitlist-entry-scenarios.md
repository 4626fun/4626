...
- **Wallet finalization**: the stage where the user's execution wallet becomes ready on the appropriate track. For CSW users (`executionMode === 'canonical'`), this means the canonical CSW is resolved and the Privy embedded EOA is confirmed as an owner/signing authority for that parent CSW (`legacy-owner-install` / `canonical4337`) per [docs/_internal/4626-connection-methods.md](../../docs/_internal/4626-connection-methods.md). For external EOA users (`executionMode === 'eoa'`), this means the EOA is connected via wagmi. The separate server-side owner-delegation track (deploy-session, agent) is governed by `.cursor/rules/csw-agent-lifecycle.mdc`.
...
4. Wallet-dependent actions stay gated until the appropriate execution-track readiness check succeeds — parent CSW + embedded-owner ready for CSW users, or connected EOA for external-EOA users, per [docs/_internal/4626-connection-methods.md](../../docs/_internal/4626-connection-methods.md).
...
4. **Wallet finalization**: canonical CSW resolution plus embedded-owner install (CSW track) or external EOA connection (EOA track) must remain explicit post-auth steps.
5. **Deploy/session coupling**: deploy/session flows must not regress to single-provider or wallet-first authentication prompts. Deploy-session continues to use direct owner delegation (server-side track) and is separate from user-initiated parent-CSW signing setup.
