/**
 * Core tombstone-aware + alias-chasing profile ID resolution.
 *
 * This module is the canonical implementation for resolving 4626 profiles
 * from Privy user IDs while correctly handling:
 * - `privy_user_aliases` (alias wins over direct `profiles.privy_user_id`)
 * - `profiles.merged_into_profile_id` (tombstone chasing)
 *
 * New code should import via `@4626/server-core`, not directly from server/_lib.
 */
