...
const DEPLOYMENT_BATCHER_PHASE_ABI = [
  ...
  // Decode-only: legacy extended finalizePhase2 (meteoraAlphaVault + solanaIxs).
  {
    type: 'function',
    name: 'finalizePhase2',
    stateMutability: 'payable',
...
// Legacy (pre-v1.19.1) selectors kept for in-flight / historical allowlisting.
const SELECTOR_BATCHER_DEPLOY_PHASE1_LEGACY = '0x3c51ca4e'
const SELECTOR_BATCHER_DEPLOY_PHASE1_WITH_SALT_LEGACY = '0x297cb1e6'
const SELECTOR_BATCHER_DEPLOY_PHASE1_CORE_LEGACY = '0x1331378b'
const SELECTOR_BATCHER_DEPLOY_PHASE1_CORE_WITH_SALT_LEGACY = '0x4154f24e'
const SELECTOR_BATCHER_FINALIZE_PHASE1_LEGACY = '0xa98ec9d8'
const SELECTOR_BATCHER_FINALIZE_PHASE1_WITH_SALT_LEGACY = '0x3bc09a8b'
// v1.19.1 DeploymentBatcher ABI selectors (from DEPLOYMENT_BATCHER_ABI).
const SELECTOR_BATCHER_DEPLOY_PHASE1 = '0x1c3e4d75'
const SELECTOR_BATCHER_DEPLOY_PHASE1_WITH_SALT = '0x74b4884e'
const SELECTOR_BATCHER_DEPLOY_PHASE1_CORE = '0x37ce9666'
const SELECTOR_BATCHER_DEPLOY_PHASE1_CORE_WITH_SALT = '0x8287b529'
const SELECTOR_BATCHER_FINALIZE_PHASE1 = '0xe93fc211'
...
// - legacy extended (meteoraAlphaVault + solanaIxs) decode/allowlist: 0xbd4583fb / 0xab56c176
const SELECTOR_BATCHER_FINALIZE_PHASE2 = '0xcafc9348'
const SELECTOR_BATCHER_FINALIZE_PHASE2_WITH_PERMIT2 = '0x8e782ae1'
const SELECTOR_BATCHER_FINALIZE_PHASE2_LEGACY = '0xbd4583fb'
const SELECTOR_BATCHER_FINALIZE_PHASE2_WITH_PERMIT2_LEGACY = '0xab56c176'
...
const ALLOWED_BATCHER_SELECTORS = new Set<string>([
  ...
  SELECTOR_BATCHER_DEPLOY_PHASE1_LEGACY,
  SELECTOR_BATCHER_DEPLOY_PHASE1_WITH_SALT_LEGACY,
  SELECTOR_BATCHER_DEPLOY_PHASE1_CORE_LEGACY,
  SELECTOR_BATCHER_DEPLOY_PHASE1_CORE_WITH_SALT_LEGACY,
  SELECTOR_BATCHER_FINALIZE_PHASE1_LEGACY,
  SELECTOR_BATCHER_FINALIZE_PHASE1_WITH_SALT_LEGACY,
  ...
  SELECTOR_BATCHER_FINALIZE_PHASE2_WITH_PERMIT2_LEGACY,
...
function readActivationOwnerPolicyHeader(req: VercelRequest): string | null {
  const raw =
    req.headers?.['x-cv-activation-owner-policy'] ??
    req.headers?.['X-CV-Activation-Owner-Policy']
  const token =
    typeof raw === 'string'
      ? raw.trim()
...
const PAYOUT_ROUTER_CODE_ID = keccak256(DEPLOY_BYTECODE.CreatorPayoutRouter as Hex)
const AGENT_REVENUE_ROUTER_CODE_ID = keccak256(DEPLOY_BYTECODE.AgentRevenueRouter as Hex)
...
const AGENT_REVENUE_POLICY_CONTROLLER_CODE_ID = keccak256(DEPLOY_BYTECODE.AgentRevenuePolicyController as Hex)
const AGENT_OVAULT_CODE_ID = keccak256(DEPLOY_BYTECODE.AgentOVault as Hex)
...
// Mirrors PayoutRouter.DEFAULT_PROTOCOL_REWARDS — see contracts/utilities/routers/CreatorPayoutRouter.sol.
...
async function validateActivationOwnerBinding(params: {
  ...
  if (params.token.sessionAddress !== params.sessionAddress) {
    throw new Error('activation_owner_policy_session_mismatch')
  }
  if (params.token.embeddedOwnerAddress !== params.sessionAddress) {
    throw new Error('activation_owner_policy_embedded_mismatch')
  }
  if (params.token.smartWalletAddress !== params.sender) {
    throw new Error('activation_owner_policy_sender_mismatch')
  }
  if (params.innerCalls.length !== 1) {
    throw new Error('activation_owner_policy_call_count_invalid')
  }
  const call = params.innerCalls[0]
  if (!call || call.target !== params.sender || call.value !== 0n) {
    throw new Error('activation_owner_policy_self_call_invalid')
  }
  const decoded = decodeFunctionData({
    abi: COINBASE_SMART_WALLET_OWNER_MGMT_ABI,
    data: call.data,
  ...
  if (
    decoded.functionName !== 'addOwnerAddress' ||
    getAddress(decoded.args[0] as Address) !== params.token.serverOwnerAddress
  ) {
    throw new Error('activation_owner_policy_owner_mismatch')
  }

  const db = isDbConfigured() ? await getDb() : null
  if (!db) throw new Error('activation_owner_policy_db_unavailable')
  const result = await (db as any).sql`
    SELECT id, privy_user_id, csw_address, primary_embedded_eoa,
           preprov_server_wallet_address
    FROM profiles
    WHERE id = ${params.token.profileId}
      AND merged_into_profile_id IS NULL
    LIMIT 1;
  `
  const row = result.rows?.[0] as Record<string, unknown> | undefined
  const bindingMatches =
    String(row?.privy_user_id ?? '') === params.token.privyUserId &&
    isAddress(String(row?.csw_address ?? '')) &&
    getAddress(String(row?.csw_address)) === params.sender &&
    isAddress(String(row?.primary_embedded_eoa ?? '')) &&
    getAddress(String(row?.primary_embedded_eoa)) === params.sessionAddress &&
    isAddress(String(row?.preprov_server_wallet_address ?? '')) &&
  ...
  if (!embeddedOwnerConfirmed) throw new Error('activation_embedded_owner_not_confirmed')
}
...
export async function validateSponsoredSmartWalletCalls(params: {
  ...
  const activationOwnerPolicyToken =
    ...
    params.activationOwnerPolicyToken.trim()
      ? params.activationOwnerPolicyToken.trim()
      : null
  const activationOwnerSponsorship = activationOwnerPolicyToken
    ? readActivationOwnerToken(activationOwnerPolicyToken)
    : null
  if (activationOwnerPolicyToken && !activationOwnerSponsorship) {
    throw new Error('activation_owner_policy_invalid')
  }
  if (activationOwnerSponsorship) {
    await validateActivationOwnerBinding({
      token: activationOwnerSponsorship,
      sender: params.sender,
      sessionAddress: params.sessionAddress,
      innerCalls,
    })
  ...
  if (!customOwnerSponsorship && !activationOwnerSponsorship) {
...
async function validateInnerCalls(params: {
  ...
  activationOwnerSponsorship?: DecodedActivationOwnerToken | null
  debug?: (info: {
    deployer: Address
    storeEnv: Address | null
    storeFromDeployer: Address | null
  ...
  const alfaClubLp = await validateAlfaClubLpCalls({
    calls: innerCalls,
    sender: params.sender,
    client: await getBaseClient(),
    env: process.env,
  })
  if (alfaClubLp) {
    return { expectedCreatorToken: alfaClubLp.creatorCoin, mode: 'alfaclub_lp' }
  }
    ...
    if (c.target === deploymentBatcher) {
      ...
      if (
        ...
        if (isPhase2DeploySelector) {
          ...
          const matchesPhase2Lane = (vaultKind: VaultKind): boolean => {
            const lane = resolveDeployLanePhase1CodeIds(vaultKind)
            return (
              !!codeIds &&
              ...
              String(codeIds.vault).toLowerCase() === String(lane.vault).toLowerCase() &&
              String(codeIds.wrapper).toLowerCase() === String(lane.wrapper).toLowerCase() &&
              String(codeIds.shareOFT).toLowerCase() === String(lane.shareOFT).toLowerCase() &&
              String(codeIds.gauge).toLowerCase() === String(lane.gauge).toLowerCase() &&
              String(codeIds.cca).toLowerCase() === String(lane.cca).toLowerCase() &&
              String(codeIds.oracle).toLowerCase() === String(lane.oracle).toLowerCase() &&
              String(codeIds.oftBootstrap).toLowerCase() === String(lane.oftBootstrap).toLowerCase()
            )
          }
          // Accept either Creator or Agent phase-1 code-id sets; reject mixed/cross-lane tuples.
          if (!matchesPhase2Lane('creator') && !matchesPhase2Lane('agent')) {
            throw new Error('batcher_phase2_codeids_mismatch')
          }
        }
  ...
  if (mode === 'deploy_phase2' || mode === 'deploy_phase3') {
    ...
    if (mode === 'deploy_phase2') {
      ...
      const [wrapperVault, wrapperShare, wrapperVaultToken, wrapperCreatorCoin, wrapperAgentToken, wrapperOwner, shareVault, shareOwner] =
        (await Promise.all([
          ...
          client.readContract({ address: expectedWrapper, abi: WRAPPER_VIEW_ABI, functionName: 'vaultToken' }).catch(() => null),
          client.readContract({ address: expectedWrapper, abi: WRAPPER_VIEW_ABI, functionName: 'creatorCoin' }).catch(() => null),
          client.readContract({ address: expectedWrapper, abi: WRAPPER_VIEW_ABI, functionName: 'agentToken' }).catch(() => null),
      ...
      if (!wrapperAsset || wrapperAsset !== getAddress(expectedCreatorToken as Address)) {
        throw new Error('wrapper_asset_token_mismatch')
      }
    ...
    if (canValidateViaCreate2) {
      ...
      const vaultKindForCreate2: VaultKind | null =
        String(vaultCodeId).toLowerCase() === String(CREATOR_OVAULT_CODE_ID).toLowerCase()
          ? 'creator'
          : String(vaultCodeId).toLowerCase() === String(AGENT_OVAULT_CODE_ID).toLowerCase()
            ? 'agent'
            : null
      if (!vaultKindForCreate2) {
        throw new Error('vault_codeid_not_allowed')
      }
      const creationCode = resolveDeployLaneVaultBytecode(vaultKindForCreate2)
      ...
      const vaultSaltLabel = resolveDeployLaneVaultSaltLabel(vaultKindForCreate2)
      const vaultSalt = keccak256(encodePacked(['bytes32', 'string'], [baseSalt, vaultSaltLabel]))
    ...
    let auxVaultKind: VaultKind = 'creator'
    if (
      expectedCodeIds?.vault &&
      String(expectedCodeIds.vault).toLowerCase() === String(AGENT_OVAULT_CODE_ID).toLowerCase()
    ) {
      auxVaultKind = 'agent'
    } else {
      for (const auxCall of innerCalls) {
        if (!vaultAuxiliaryDeployBatcher || auxCall.target !== vaultAuxiliaryDeployBatcher) continue
        if (getSelector(auxCall.data) !== SELECTOR_VAULT_AUXILIARY_DEPLOY_PHASE2) continue
        try {
          const decoded = decodeFunctionData({
            abi: VAULT_AUXILIARY_DEPLOY_BATCHER_ABI as any,
            data: auxCall.data,
          })
          const kindRaw = (decoded as any)?.args?.[0]?.vaultKind
          const parsedKind = parseOnchainVaultKind(kindRaw)
          if (parsedKind) auxVaultKind = parsedKind
        } catch {
          // Fall back to creator until the aux call is validated below.
        }
        break
      }
    }
    const expectedRevenueRouterCodeId = resolveDeployLanePayoutRouterCodeId(auxVaultKind)
    const expectedRevenuePolicyControllerCodeId =
      resolveDeployLaneRevenuePolicyControllerCodeId(auxVaultKind)

    const burnSalt = deriveVaultShareBurnStreamSalt({
    ...
    const routerSalt = derivePayoutRouterSalt({
      creatorToken: expectedCreatorToken as Address,
      owner: params.sender,
      vaultKind: auxVaultKind,
    })
    if (!expectedProtocolTreasury) throw new Error('protocol_treasury_not_configured')
    expectedPayoutRouter = await computeCreate2AddressFromStore({
      ...
      deployer: create2DeployerFromStore,
      salt: routerSalt,
      codeId: expectedRevenueRouterCodeId as Hex,
    ...
    const policyControllerSalt = deriveRevenuePolicyControllerSalt({
      creatorToken: expectedCreatorToken as Address,
      owner: params.sender,
      vaultKind: auxVaultKind,
    })
    expectedCreatorCoinPolicyController = await computeCreate2AddressFromStore({
      ...
      codeId: expectedRevenuePolicyControllerCodeId as Hex,
  ...
  for (const c of innerCalls) {
    ...
    if (c.target === params.sender && ALLOWED_SELF_SELECTORS.has(selector)) {
      ...
      if (decodedSelf.functionName === 'addOwnerAddress') {
        ...
        if (params.activationOwnerSponsorship) {
          if (params.activationOwnerSponsorship.sessionAddress !== params.sessionAddress) {
            throw new Error('activation_owner_policy_session_mismatch')
          }
          if (params.activationOwnerSponsorship.smartWalletAddress !== params.sender) {
            throw new Error('activation_owner_policy_sender_mismatch')
          }
          if (ownerArg !== params.activationOwnerSponsorship.serverOwnerAddress) {
            throw new Error('activation_owner_policy_owner_mismatch')
          }
    ...
    if (vaultAuxiliaryDeployBatcher && c.target === vaultAuxiliaryDeployBatcher) {
      if (selector !== SELECTOR_VAULT_AUXILIARY_DEPLOY_PHASE2) throw new Error('auxiliary_batcher_selector_not_allowed')
      {
        ...
        const vaultKindArg = parseOnchainVaultKind(p?.vaultKind)
        if (!assetTokenArg || assetTokenArg !== expectedCreatorToken) throw new Error('batcher_aux_creator_mismatch')
        ...
        if (!shareOftArg || !expectedShareOFT || shareOftArg !== expectedShareOFT) {
          throw new Error('batcher_aux_shareoft_mismatch')
        }
        if (!wrapperArg || !expectedWrapper || wrapperArg !== expectedWrapper) {
          throw new Error('batcher_aux_wrapper_mismatch')
        }
        ...
        if (!vaultKindArg) throw new Error('batcher_aux_vault_kind_mismatch')
        ...
        const expectedRouterCodeId = resolveDeployLanePayoutRouterCodeId(vaultKindArg)
        const expectedPolicyCodeId = resolveDeployLaneRevenuePolicyControllerCodeId(vaultKindArg)
        // Fail closed: reject cross-lane or arbitrary auxiliary bytecode.
        if (vaultKindArg === 'creator') {
          if (
            String(expectedRouterCodeId).toLowerCase() !== String(PAYOUT_ROUTER_CODE_ID).toLowerCase() ||
            String(expectedPolicyCodeId).toLowerCase() !==
              String(CREATOR_COIN_POLICY_CONTROLLER_CODE_ID).toLowerCase()
          ) {
            throw new Error('batcher_aux_codeids_mismatch')
          }
        } else if (vaultKindArg === 'agent') {
          if (
            String(expectedRouterCodeId).toLowerCase() !== String(AGENT_REVENUE_ROUTER_CODE_ID).toLowerCase() ||
            String(expectedPolicyCodeId).toLowerCase() !==
              String(AGENT_REVENUE_POLICY_CONTROLLER_CODE_ID).toLowerCase()
          ) {
            throw new Error('batcher_aux_codeids_mismatch')
          }
        } else {
          const _exhaustive: never = vaultKindArg
          void _exhaustive
          throw new Error('batcher_aux_vault_kind_mismatch')
        }
        const hasValidAuxiliaryCodeIds =
          codeIds &&
          typeof codeIds === 'object' &&
          isHexString(codeIds.vaultShareBurnStream) &&
          isHexString(codeIds.revenueRouter) &&
          isHexString(codeIds.revenuePolicyController) &&
          String(codeIds.vaultShareBurnStream).toLowerCase() ===
            String(VAULT_SHARE_BURN_STREAM_CODE_ID).toLowerCase() &&
          String(codeIds.revenueRouter).toLowerCase() === String(expectedRouterCodeId).toLowerCase() &&
          String(codeIds.revenuePolicyController).toLowerCase() === String(expectedPolicyCodeId).toLowerCase()
...
async function handlePaymasterRequest(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const activationOwnerPolicyTokenHeader = readActivationOwnerPolicyHeader(req)
    ...
    for (const r of requests) {
      ...
      const validated = await validateSponsoredSmartWalletCalls({
        ...
        activationOwnerPolicyToken: activationOwnerPolicyTokenHeader,
