...
// Strict breakout contract: never use fallback bands.
const ALLOW_PREMIUM_FALLBACK_BAND = false
...
const PREMIUM_BREAKOUT_MASK_MAX_COVERAGE_ILLUSTRATION =
  clamp(
    Number(process.env.TOKEN_PREMIUM_BREAKOUT_MASK_MAX_COVERAGE_ILLUSTRATION ?? 0.72),
...
function getSegmentationBreakoutMaxCoverage(sourceClass?: SourceClass): number {
  ...
  if (sourceClass === 'pixelArt') {
    return Math.max(PREMIUM_BREAKOUT_MASK_MAX_COVERAGE, PREMIUM_BREAKOUT_MASK_MAX_COVERAGE_PIXEL_ART)
  }
...
async function hardenImageAlpha(image: Buffer, alphaFloor: number): Promise<Buffer> {
  const floor = clamp(Math.round(alphaFloor), 0, 254)
  ...
  for (let i = 0; i < out.length; i += info.channels) {
    ...
    out[alphaIndex] = Math.min(255, Math.round(alpha * 2.2))
  }
...
async function scrubTopDarkFringe(image: Buffer): Promise<Buffer> {
  ...
  const probeDepth = Math.max(6, Math.min(24, Math.round(bounds.height * 0.16)))
  const minRowAlpha = Math.max(24, Math.round(bounds.width * 0.08))
  let suspiciousRows = 0
  for (let y = bounds.minY; y <= Math.min(bounds.maxY, bounds.minY + probeDepth); y += 1) {
    let rowAlphaCount = 0
    let rowDarkNeutralCount = 0
    for (let x = bounds.minX; x <= bounds.maxX; x += 1) {
      ...
      if (luma < 24 && chroma < 12) rowDarkNeutralCount += 1
    }
    if (rowAlphaCount >= minRowAlpha && rowDarkNeutralCount / rowAlphaCount > 0.9) {
      suspiciousRows += 1
    }
  }

  if (suspiciousRows < 2) return image

  const out = Buffer.from(data)
  const cleanupDepth = Math.max(8, Math.min(30, Math.round(bounds.height * 0.2)))
  for (let y = bounds.minY; y <= Math.min(bounds.maxY, bounds.minY + cleanupDepth); y += 1) {
    const topProgress = (y - bounds.minY) / Math.max(1, cleanupDepth)
    const lumaCutoff = topProgress < 0.55 ? 36 : 24
    const chromaCutoff = topProgress < 0.55 ? 16 : 12
    for (let x = bounds.minX; x <= bounds.maxX; x += 1) {
      const idx = (y * info.width + x) * info.channels
      const alpha = out[idx + 3] ?? 0
      if (alpha < 10) continue
      const r = out[idx] ?? 0
      const g = out[idx + 1] ?? 0
      const b = out[idx + 2] ?? 0
      const luma = 0.2126 * r + 0.7152 * g + 0.0722 * b
      const chroma = Math.max(r, g, b) - Math.min(r, g, b)
      if (luma < lumaCutoff && chroma < chromaCutoff) {
        out[idx + 3] = 0
      }
    }
  }
...
async function analyzeSourceImage(params: {
  ...
  if (sourceClass === 'brightBadge') {
    ...
    const lowConfidenceLowResPixelArt =
      lowResolution &&
      !hasTransparency &&
      topCenterStdDev < 24 &&
      topOccupancy < 0.12
    fitMode = lowConfidenceLowResPixelArt ? 'contain' : 'cover'
    preferredScale = lowConfidenceLowResPixelArt ? 0.96 : 1.005
...
function decideBreakoutPlan(params: {
  ...
  // High-confidence runtime segmentation path for opaque assets.
  // Fallback-band remains disabled, and downstream structural/coverage gates
  // must still pass before breakout is rendered.
  const illustrationRembgCandidate =
    breakoutRequested &&
    !analysis.hasTransparency &&
    analysis.sourceClass === 'illustration' &&
    analysis.topCenterStdDev > 30 &&
    analysis.topOccupancy > 0.10 &&
    analysis.topOccupancy < 0.94
  const portraitRembgCandidate =
    breakoutRequested &&
    !analysis.hasTransparency &&
    !analysis.lowResolution &&
    analysis.sourceClass === 'portraitPhoto' &&
    analysis.topCenterStdDev > 22 &&
    analysis.topOccupancy > 0.045 &&
    analysis.topOccupancy < 0.48
  const pixelArtRembgCandidate =
    breakoutRequested &&
    !analysis.hasTransparency &&
  ...
  if (!rembgCandidate) {
    return { mode: 'none', breakoutRequested, rembgCandidate, reason: 'no-breakout-source' }
  }
...
async function measureBreakoutMaskCoverage(params: {
  ...
  sourceClass: SourceClass
  maskRgbaPng: Buffer
  maskKind?: 'heroCutout' | 'sourceAlpha' | 'rembgCutout'
}): Promise<number> {
  const maskCanvas = await renderPlacedSourceCanvas({
    sourceImage: params.maskRgbaPng,
    layout: params.layout,
  ...
  for (const layer of stackConfig.layers) {
    ...
    if (useSubjectSilhouetteSpill) {
      ...
      silhouette = await sharp(silhouette)
        ...
        .png()
        .toBuffer()
      const spillOpacityScale =
        params.clipRegion === 'extended'
          ? 1.38
          : params.clipRegion === 'padding-outside-frame'
            ? 1.28
            : 1.08
      renderedLayers.push(await applyOpacity(silhouette, layer.opacity * spillOpacityScale))
      continue
    }
...
function resolvePreparedBreakoutTopLiftRatio(params: {
  maskKind?: 'heroCutout' | 'sourceAlpha' | 'rembgCutout'
  sourceClass?: SourceClass
}): number {
  const isPrepared = params.maskKind === 'heroCutout' || params.maskKind === 'rembgCutout'
  if (!isPrepared) return 0
  switch (params.sourceClass) {
    case 'pixelArt':
      return 0.3
    case 'illustration':
      return 0.52
...
async function createTopBreakoutMask(params: {
  ...
  const topLiftRatio = resolvePreparedBreakoutTopLiftRatio({
    maskKind: params.maskKind,
    sourceClass: params.sourceClass,
  })
  const top = isHeroCutout
    ? 0
    : topLiftRatio > 0
      ? Math.max(0, layout.breakoutY - Math.round(layout.breakoutHeight * topLiftRatio))
      : layout.breakoutY
  const rembgDepthMultiplier =
    isPreparedCutout
      ? (params.sourceClass === 'pixelArt' ? (isHeroCutout ? 1.62 : 1.78) : params.sourceClass === 'illustration' ? 1.28 : 1.16)
      : 1
  const depthBottom = top + Math.round(layout.breakoutHeight * rembgDepthMultiplier)
  const overlapIntoChamberPx = Math.max(1, Math.round(layout.frameStroke * (isHeroCutout && isIllustration ? 0.72 : 0.5)))
  const bottom = isHeroCutout && isIllustration
    ? Math.min(size, Math.max(depthBottom, layout.chamberY + overlapIntoChamberPx))
    : Math.min(size, depthBottom)
  ...
  <defs>
    <linearGradient id="fadeY" x1="0" y1="${top}" x2="0" y2="${bottom}" gradientUnits="userSpaceOnUse">
      ...
      <stop offset="${lowerFadeStop}" stop-color="white" stop-opacity="${lowerFadeOpacity}"/>
      <stop offset="100%" stop-color="white" stop-opacity="${bottomFadeOpacity}"/>
    </linearGradient>
...
function selectDominantUpperSubjectComponent(params: {
  mask: Buffer
  width: number
  height: number
  x0: number
  ...
  const isPixelArt = params.sourceClass === 'pixelArt'
  const isIllustration = params.sourceClass === 'illustration'
  const { mask, width, height, x0, x1, y0, y1, layout } = params
  const regionArea = Math.max(1, (x1 - x0) * (y1 - y0))
  const upperLimitY = Math.min(
    height - 1,
  ...
  for (let y = y0; y < y1; y += 1) {
    for (let x = x0; x < x1; x += 1) {
      ...
      if ((mask[seedIdx] ?? 0) === 0 || visited[seedIdx] !== 0) continue
      ...
      while (queuePos < queue.length) {
        ...
        if (px > x0) {
          const leftIdx = idx - 1
          if ((mask[leftIdx] ?? 0) !== 0 && visited[leftIdx] === 0) {
            visited[leftIdx] = 1
            queue.push(leftIdx)
          }
        }
        if (px < x1 - 1) {
          ...
          if ((mask[rightIdx] ?? 0) !== 0 && visited[rightIdx] === 0) {
            visited[rightIdx] = 1
            queue.push(rightIdx)
          }
        }
        if (py > y0) {
          ...
          if ((mask[upIdx] ?? 0) !== 0 && visited[upIdx] === 0) {
            visited[upIdx] = 1
            queue.push(upIdx)
          }
        }
        if (py < y1 - 1) {
          ...
          if ((mask[downIdx] ?? 0) !== 0 && visited[downIdx] === 0) {
            visited[downIdx] = 1
            queue.push(downIdx)
          }
        }
      ...
      if (area < minComponentPixels || topHits === 0) continue
      const topRatio = topHits / area
      const areaRatio = area / regionArea
      const widthRatio = (maxX - minX + 1) / Math.max(1, x1 - x0)
      const heightRatio = (maxY - minY + 1) / Math.max(1, y1 - y0)
      // Reject obvious top strips/bands even in the upper-subject mode.
      if (widthRatio > 0.98 && topRatio > 0.6 && heightRatio < 0.22) continue
      if (isIllustration && topRatio < 0.26) continue

      const centroidX = sumX / area
      const centerOffsetNorm = Math.abs(centroidX - chamberCenterX) / Math.max(1, layout.chamberSize * 0.5)
      const centerScore = 1 - clamp(centerOffsetNorm, 0, 1)
      const score = areaRatio * 0.56 + topRatio * 0.24 + centerScore * 0.2
      candidates.push({
        score,
        area,
...
async function createTopBreakoutSubjectMask(params: {
  ...
  breakoutWindow?: { x: number; width: number }
  preferUpperSubjectMatte?: boolean
  maskKind?: 'heroCutout' | 'sourceAlpha' | 'rembgCutout'
}): Promise<Buffer | null> {
  ...
  const preferUpperSubjectMatte = Boolean(params.preferUpperSubjectMatte)
  ...
  const preferUpperMatteRegion = forceAlphaMask && preferUpperSubjectMatte
  const chamberPadX = Math.max(6, Math.round(layout.chamberSize * (isPixelArt ? 0.06 : 0.08)))
  const x0 = preferUpperMatteRegion
    ? Math.max(0, layout.chamberX - chamberPadX)
    : Math.max(0, breakoutWindowX + regionShiftX - regionPadX)
  const x1 = preferUpperMatteRegion
    ? Math.min(width, layout.chamberX + layout.chamberSize + chamberPadX)
    : Math.min(width, breakoutWindowX + regionShiftX + breakoutWindowWidth + regionPadX)
  const preparedTopLift = resolvePreparedBreakoutTopLiftRatio({
    maskKind: params.maskKind,
    sourceClass: params.sourceClass,
  })
  const y0 = isHeroCutout
    ? Math.max(0, layout.breakoutY - Math.round(layout.breakoutHeight * 0.58))
    : preparedTopLift > 0
      ? Math.max(0, layout.breakoutY - Math.round(layout.breakoutHeight * (preparedTopLift + 0.08)))
      : Math.max(0, layout.breakoutY - regionPadY)
  const y1 = preferUpperMatteRegion
    ? Math.min(height, layout.chamberY + Math.round(layout.chamberSize * (isPixelArt ? 0.64 : 0.60)))
    : Math.min(height, layout.breakoutY + Math.round(layout.breakoutHeight * yExtendMul))
  ...
  if (forceAlphaMask && params.preferUpperSubjectMatte) {
    const dominantComponent = selectDominantUpperSubjectComponent({
      mask,
      width,
      height,
      x0,
    ...
    if (!dominantComponent || dominantComponent.length === 0) {
      if (BREAKOUT_DEBUG_LOG_ENABLED) {
        console.info('[breakout-debug]', JSON.stringify({
          step: 'createTopBreakoutSubjectMask:decision',
          accepted: false,
          reason: 'no-dominant-upper-component',
    ...
    mask.fill(0)
    for (const idx of dominantComponent) {
      mask[idx] = 255
    }
    contourConstrained = true
    recomputeMaskStats()
  ...
  if (!forceAlphaMask) {
    ...
    if (
      presentRatio < 0.002 ||
      (!isHeroCutout && presentRatio > 0.92) ||
      heightRatio < 0.06 ||
      (strictContourGates && (
        (widthRatio > 0.98 && (topWeightedRatio > 0.52 || flatnessRatio > 2.6)) ||
        flatnessRatio > 4.8 ||
  ...
  if (forceAlphaMask && preferUpperSubjectMatte && params.sourceClass === 'illustration') {
    processedSharp = processedSharp.erode(1)
  }
  if (forceAlphaMask && preferUpperSubjectMatte && params.sourceClass === 'pixelArt') {
    processedSharp = processedSharp.dilate(1)
  }
...
export async function renderBreakoutLayer(params: {
  ...
  if (params.subjectMaskSourceImage && params.subjectMaskSourceImage.length > 0) {
    try {
      ...
      subjectRefCanvas = await renderPlacedSourceCanvas({
        sourceImage: rembgHardenedSubjectRef,
        layout,
        scale: breakoutScale,
        fit: 'cover',
        topBiasPx: params.topBiasPx,
  ...
  const subjectMask = await createTopBreakoutSubjectMask({
    ...
    layout,
    sourceClass: params.sourceClass,
    maskKind: params.subjectMaskKind,
  ...
  if ((params.subjectMaskKind === 'rembgCutout' || params.subjectMaskKind === 'heroCutout') && subjectMask) {
    ...
    if (subjectBounds) {
      ...
      const nextWidth = Math.round(clamp(desiredWidth, minWidth, maxWidth))
      const desiredCenterX = (desiredX + desiredRight) / 2
      const nextX = Math.round(clamp(desiredCenterX - nextWidth / 2, minXAllowed, maxRightAllowed - nextWidth))
      breakoutWindowForMask = { x: nextX, width: nextWidth }
      if (BREAKOUT_DEBUG_LOG_ENABLED) {
        console.info('[breakout-debug]', JSON.stringify({
          step: 'renderBreakoutLayer:window-from-subject-mask',
          sourceClass: params.sourceClass ?? 'unknown',
          subjectBounds,
          breakoutWindowSeed: breakoutWindow,
          breakoutWindowForMask,
          minWidth,
          maxWidth,
        }))
      }
  ...
  const breakoutMask = await createTopBreakoutMask({
    ...
    layout,
    sourceClass: params.sourceClass,
    breakoutWindow: breakoutWindowForMask ?? undefined,
    maskKind: params.subjectMaskKind,
  })
  const aboveFrameMask = await createBreakoutAboveFrameMask({
    size,
  ...
  const shouldNudgePreparedBreakout =
    (params.subjectMaskKind === 'rembgCutout' || params.subjectMaskKind === 'heroCutout') &&
    (params.sourceClass === 'pixelArt' || params.sourceClass === 'illustration')
  if (shouldNudgePreparedBreakout) {
    ...
    if (bounds) {
      ...
      const topLiftTarget =
        params.sourceClass === 'illustration'
          ? 0.62
          : 0.36
      const targetTopY = Math.max(0, layout.breakoutY - Math.round(layout.breakoutHeight * topLiftTarget))
      ...
      const shiftY = skipVerticalNudge
        ? 0
        : Math.round(clamp(targetTopY - bounds.minY, -maxShiftY, maxShiftY))
  ...
  if (isIllustrationBreakout && params.subjectMaskKind === 'rembgCutout') {
    ...
    const minHeightRatio = 0.10
    const minAreaRatio = 0.01
...
export async function buildPremiumSubjectStack(params: PremiumTokenIconParams): Promise<PremiumSubjectStack> {
  ...
  if (normalizedSource && analysis) {
    ...
    if (allowBreakoutForLayout) {
        ...
        breakoutPlan.mode === 'heroCutout'
          ...
          : analysis.sourceClass === 'illustration'
            ...
            : analysis.sourceClass === 'portraitPhoto'
              ? 0.34
              : 0.22
      const breakoutTopBiasPx =
        breakoutPlan.mode === 'heroCutout' && analysis.sourceClass === 'illustration'
          ? topBiasPx
          : analysis.sourceClass === 'illustration'
      ...
      const fallbackBandAllowedBySignal = false
