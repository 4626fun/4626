...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const g = await guardAgentApiRequest({ req, res, endpoint: 'v1/build/charm/setUseZRouter', kind: 'build' })
  if (!g.ok) return

  const body = (await readJsonBody<{ strategy: Address; useZRouter: boolean }>(req)) ?? ({} as any)
  try {
    const strategy = requireAddress(body.strategy, 'strategy')
    const useZRouter = requireBoolean(body.useZRouter, 'useZRouter')
    ...
    const out: BuildTxResponse = {
      ...
      data,
      value: '0',
      description: 'CreatorCharmStrategy (owner): toggle zRouter vs Uniswap router.',
