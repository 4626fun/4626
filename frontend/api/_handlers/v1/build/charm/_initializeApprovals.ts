...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const limiter = await checkDurableRateLimit(
    rateLimitKey('v1-build-charm-initialize-approvals', g.auth?.address?.toLowerCase() ?? 'anon', getClientIp(req)),
  ...
  try {
    ...
    const out: BuildTxResponse = {
      ...
      data,
      value: '0',
      description: 'CharmStrategy4626 (owner): initialize token approvals (Uniswap/charm).',
      warnings: [
