...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const g = await guardAgentApiRequest({ req, res, endpoint: 'v1/build/charm/setSwapPool', kind: 'build' })
  if (!g.ok) return

  const limiter = await checkDurableRateLimit(
    rateLimitKey('v1-build-charm-set-swap-pool', g.auth?.address?.toLowerCase() ?? 'anon', getClientIp(req)),
  ...
  try {
    ...
    const swapPool = requireAddress(body.swapPool, 'swapPool')
