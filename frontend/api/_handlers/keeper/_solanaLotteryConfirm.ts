/** POST /api/keeper/solana/lottery-confirm — machine-auth LZ receipt reconciliation. */
...
import { confirmSolanaLotteryInboxBatch } from '../../../server/_lib/onchain/solanaLotteryConfirmWorker.js'

function enabled(): boolean {
  return ['1', 'true', 'yes'].includes(String(process.env.SOLANA_LOTTERY_CONFIRM_ENABLED ?? '').trim().toLowerCase())
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!enabled()) return res.status(503).json({ success: false, error: 'solana_lottery_confirm_disabled' } satisfies ApiEnvelope<never>)
  ...
  try {
    const result = await confirmSolanaLotteryInboxBatch({ db: db as any, limit })
