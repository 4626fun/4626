...
const APPROVE_BODY_MAX_BYTES = 16_384
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const rate = checkRateLimit(
    rateLimitKey('admin-creator-access-approve', admin.toLowerCase(), getClientIp(req)),
  ...
  const body = asObjectBody(await readBoundedJsonObjectBody(req, { maxBytes: APPROVE_BODY_MAX_BYTES }))
