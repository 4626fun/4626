...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const bodyRaw = await readBoundedJsonObjectBody(req, { maxBytes: 1_024 })
  const body = (bodyRaw && typeof bodyRaw === 'object' && !Array.isArray(bodyRaw) ? bodyRaw : {}) as MaintenanceBody
  ...
  try {
    const result = await createVaultControlPlane().runMaintenanceCycle(vaultAddress)
    ...
    const message = error instanceof Error ? error.message : 'control_plane_maintenance_failed'
