...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const parentCswAddress = getAddress(bootstrap.canonicalCswAddress) as `0x${string}`
    const targetOverride = parseAddress(body.targetCswAddress)

    let cswAddress: `0x${string}` = parentCswAddress
    if (targetOverride) {
      if (targetOverride.toLowerCase() === parentCswAddress.toLowerCase()) {
        cswAddress = targetOverride as `0x${string}`
      } else {
        ...
        const allowedSub = getAddress(persistedSub) as `0x${string}`
    ...
    const ownerToAdd = getAddress(bootstrap.privyEmbeddedEoaAddress) as `0x${string}`
    if (
    ...
    const relayQuoteUser = getAddress(
      relayFundingCswHint &&
    ...
    const response: AddOwnerPreviewResponse = {
      ...
      preflight: {
        ownerToAdd: ownerToAdd as `0x${string}`,
        alreadyOwner: false,
        simulation,
