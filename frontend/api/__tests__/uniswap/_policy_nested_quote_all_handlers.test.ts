...
describe('uniswap handlers block nested quote token bypass', () => {
  ...
  it('rejects nested denied token in /api/uniswap/order quote', async () => {
    ...
    const req = createMockReq({
      method: 'POST',
      headers: withAuthHeader({ origin: 'https://v1.4626.fun', 'x-forwarded-for': '10.2.0.1' }),
      body: {
        signature: '0xabc',
        quote: {
  ...
  it('rejects nested denied token in /api/uniswap/swap_5792 classicQuote', async () => {
    ...
    const req = createMockReq({
      method: 'POST',
      headers: withAuthHeader({ origin: 'https://v1.4626.fun', 'x-forwarded-for': '10.2.0.2' }),
      body: {
  ...
  it('rejects nested denied token in /api/uniswap/swap_7702 classicQuote', async () => {
    ...
    const req = createMockReq({
      method: 'POST',
      headers: withAuthHeader({ origin: 'https://v1.4626.fun', 'x-forwarded-for': '10.2.0.3' }),
      body: {
