...
vi.mock('../../src/deploy/bytecode.generated.js', () => ({
  DEPLOY_BYTECODE: {
    CreatorOVault: ('0x' + '01'.repeat(32)) as `0x${string}`,
    AgentOVault: ('0x' + '02'.repeat(32)) as `0x${string}`,
    CreatorOVaultWrapper: ('0x' + '03'.repeat(32)) as `0x${string}`,
    AgentOVaultWrapper: ('0x' + '04'.repeat(32)) as `0x${string}`,
    CreatorShareOFT: ('0x' + '05'.repeat(32)) as `0x${string}`,
    AgentShareOFT: ('0x' + '06'.repeat(32)) as `0x${string}`,
    OFTBootstrapRegistry: ('0x' + '07'.repeat(32)) as `0x${string}`,
    CreatorGaugeController: ('0x' + '08'.repeat(32)) as `0x${string}`,
    AgentGaugeController: ('0x' + '09'.repeat(32)) as `0x${string}`,
    CCALaunchArm: ('0x' + '0a'.repeat(32)) as `0x${string}`,
    CreatorOracle: ('0x' + '0b'.repeat(32)) as `0x${string}`,
    AgentOracle: ('0x' + '0c'.repeat(32)) as `0x${string}`,
    CreatorPayoutRouter: ('0x' + '0d'.repeat(32)) as `0x${string}`,
    AgentRevenueRouter: ('0x' + '0e'.repeat(32)) as `0x${string}`,
    VaultShareBurnStream: ('0x' + '0f'.repeat(32)) as `0x${string}`,
    CreatorCoinPolicyController: ('0x' + '10'.repeat(32)) as `0x${string}`,
    AgentRevenuePolicyController: ('0x' + '11'.repeat(32)) as `0x${string}`,
    CharmStrategy4626: ('0x' + '12'.repeat(32)) as `0x${string}`,
    AjnaVaultAuth: ('0x' + '13'.repeat(32)) as `0x${string}`,
    AjnaERC4626Vault: ('0x' + '14'.repeat(32)) as `0x${string}`,
    ERC4626StrategyAdapter: ('0x' + '15'.repeat(32)) as `0x${string}`,
  },
}))
...
describe('paymaster deploy-session setup (selfcall-only)', () => {
  ...
  it('rejects addOwnerAddress self-call for deploy_session_setup when deploy session is missing', async () => {
    ...
    // Without deploy-session binding or canonical embedded-owner binding, sponsorship is denied.
  ...
  it('accepts addOwnerAddress self-call when owner matches canonical persisted embedded EOA without deploy session', async () => {
    const COINBASE_SMART_WALLET_OWNER_MGMT_ABI = [
      {
        type: 'function',
        name: 'addOwnerAddress',
    ...
    await expect(
      validateSponsoredSmartWalletCalls({
        ...
        sessionAddress,
        calls: [{ to: sender, value: 0n, data: innerData }],
        canonicalEmbeddedOwner: embeddedOwner,
      }),
  ...
  it('accepts handler addOwnerAddress self-call when canonical embedded owner resolves via session profile authority', async () => {
    const COINBASE_SMART_WALLET_ABI = [
      {
        type: 'function',
        name: 'execute',
    ...
    const embeddedOwner = getAddress('0x7777777777777777777777777777777777777777')
    ...
    resolvePersistedWalletIdentityMock.mockImplementation(async (address: string) => {
      if (String(address).toLowerCase() === sender.toLowerCase()) {
        senderLookupCount += 1
        if (senderLookupCount === 1) return null
        return {
          canonicalSmartWallet: sender,
          embeddedEoa: embeddedOwner,
        }
    ...
    const identityLookupAddresses = resolvePersistedWalletIdentityMock.mock.calls.map((call) =>
  ...
  it('rejects addOwnerAddress self-call when canonical embedded-owner binding mismatches owner arg', async () => {
    ...
    ).rejects.toThrow(/canonical_embedded_owner_mismatch/i)
  })
