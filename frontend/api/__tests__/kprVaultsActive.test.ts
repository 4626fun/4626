...
import kprHandler from '../_handlers/vaults/_activeProtected.ts'
...
const {
  ...
  validateKeeperVaultListingMock: vi.fn(async () => ({ ok: true as const })),
}))
...
describe('/api/vaults/active automation exposure', () => {
  let restoreEnv: (() => void) | null = null

  beforeEach(() => {
    vi.clearAllMocks()
    validateKeeperVaultListingMock.mockImplementation(async () => ({ ok: true }))
