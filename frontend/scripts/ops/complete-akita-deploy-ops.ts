...
function requireAddresses(): DeployAddresses {
  ...
  out.cca = isAddress(getArg('--cca'))
    ...
    : (AKITA_DEFAULTS.ccaLaunchArm as Address)
...
function printComposerBlock(addrs: DeployAddresses): void {
  ...
  process.stdout.write(
    'AKITA launch note: Pipe B compose-deposit is dormant; configureTokenMesh is deferred and\n' +
  ...
  process.stdout.write(
    'INPUT REQUIRED before Safe calldata can be generated (future activation only):\n' +
      '  configureTokenMesh reverts on zero assetMeshToken/solanaAssetPeer (OVaultHubComposer ZeroAddress guard),\n' +
...
function writeDefaultsPatch(addrs: DeployAddresses): void {
  ...
  replaceAddr('ccaLaunchArm', AKITA_DEFAULTS.ccaLaunchArm.slice(2), addrs.cca)
