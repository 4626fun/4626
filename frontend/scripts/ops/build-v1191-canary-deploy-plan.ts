...
import { resolveDeployExpectedAddresses, type ResolveDeployExpectedAddressesResult } from '../../src/lib/deploy/resolveDeployExpectedAddresses.js'
import {
...
import { attachFinalizeShareBridgeValueToCalls, parseCallValue } from '../../src/lib/deploy/finalizeShareBridgeFee.js'
import { buildImpairmentAuxPlan, PERMISSIONLESS_CREATE2_DEPLOYER } from '../../src/lib/deploy/impairmentAuxPlan.js'
...
const DEPLOYMENT_BATCHER = getAddress('0xa18169caf37fa0347285B16aAFC2B09eCB43F145')
const VAULT_AUXILIARY_DEPLOY_BATCHER = getAddress('0xaA9229c1649a7eC6DA85a76097E0910B24F9408e')
...
function usage(): string {
  return [
    'Usage:',
    '  pnpm -C frontend exec tsx --env-file=.env scripts/ops/build-v1191-canary-deploy-plan.ts \\',
    '    --vault-kind creator|agent --creator-token 0x... --out path.json',
  ].join('\n')
}
...
function parseArgs(argv: string[]): Args {
  ...
  const vaultKindRaw = String(getArg('--vault-kind') ?? '').trim().toLowerCase()
  ...
  if (vaultKindRaw !== 'creator' && vaultKindRaw !== 'agent') {
    throw new Error(`Invalid --vault-kind: ${vaultKindRaw || '(empty)'}\n${usage()}`)
  }
  if (!isAddress(creatorTokenRaw)) {
    throw new Error(`Invalid --creator-token: ${creatorTokenRaw || '(empty)'}\n${usage()}`)
  }
  if (!out) {
...
function serializeSessionCalls(calls: Array<{ target: Address; value: bigint; data: Hex }>): DeploySessionCall[] {
...
function encodeUniswapCcaLinearSteps(durationBlocks: bigint): Hex {
  ...
  const packStep = (mps: bigint, blockDelta: bigint) =>
    encodeAbiParameters(
      parseAbiParameters('uint24 mps, uint40 blockDelta'),
      [Number(mps), Number(blockDelta)],
    ).replace(/^0x/, '').slice(0, 6 + 10) as string
...
async function readTokenMetadata(
  ...
  const [symbol, name, decimals] = await Promise.all([
    publicClient.readContract({ address: creatorToken, abi: erc20Abi, functionName: 'symbol' }) as Promise<string>,
    publicClient.readContract({ address: creatorToken, abi: erc20Abi, functionName: 'name' }) as Promise<string>,
    publicClient.readContract({ address: creatorToken, abi: erc20Abi, functionName: 'decimals' }) as Promise<number>,
  ])
...
function deriveTokenNamesAndSymbols(
  token: TokenMetadata,
  vaultKind: TokenSymbolVaultKind,
): { vaultName: string; vaultSymbol: string; shareName: string; shareSymbol: string } {
  const underlying = normalizeUnderlyingSymbol(token.symbol)
  const underlyingUpper = underlying ? deriveUnderlyingUpper(underlying) : deriveUnderlyingUpper(token.symbol)
  if (!underlyingUpper) {
    throw new Error(`Could not derive token symbol from creator token symbol "${token.symbol}"`)
  }
  return {
...
async function resolveVanityPlanLite(params: {
  ...
  let shareOftSaltOverrideUsed: Hex | null = null
  const requestedSuffix = normalizeHexSuffix(shareOftVanitySuffix)
  if (requestedSuffix && !deterministicShareAddress.toLowerCase().endsWith(requestedSuffix)) {
  ...
  return {
    ...
    vanityVersionSearchOutcome: shareOftSaltOverrideUsed ? 'vault_only_match' : 'combined_match',
...
async function resolveExpectedAddressesForPlan(params: {
  ...
  vaultKind: TokenSymbolVaultKind
  token: TokenMetadata
  names: { vaultName: string; vaultSymbol: string; shareName: string; shareSymbol: string }
}): Promise<{
  ...
  const infraResult = await readCreatorVaultBatcherInfra({
    ...
    fallbacks: {
      create2Deployer: normalizeAddressLike(contracts.universalCreate2DeployerFromStore ?? null),
      bytecodeStore: normalizeAddressLike(contracts.universalBytecodeStore ?? null),
      protocolTreasury: normalizeAddressLike(contracts.protocolTreasury ?? null),
  ...
  const expectedAddresses = await resolveDeployExpectedAddresses({
    ...
    chainId: base.id,
    vanityPlan,
    universalBytecodeStoreFallback: normalizeAddressLike(contracts.universalBytecodeStore ?? null),
    wethAddress: getAddress(contracts.weth ?? BASE_DEFAULTS.weth),
    vaultShareBurnStreamCodeId: keccak256(DEPLOY_BYTECODE.VaultShareBurnStream as Hex),
    payoutRouterCodeId: resolveDeployLanePayoutRouterCodeId(params.vaultKind),
    creatorCoinPolicyControllerCodeId: resolveDeployLaneRevenuePolicyControllerCodeId(params.vaultKind),
    vaultKind: params.vaultKind,
  })
...
async function main(): Promise<void> {
  ...
  const { contracts, expectedAddresses, vanityPlan } = await resolveExpectedAddressesForPlan({
    publicClient,
  ...
  const expectedCreatorCoinPolicyController = expected.creatorCoinPolicyController
  const expectedProtocolTreasury = getAddress(expectedAddresses.protocolTreasury)
  ...
  const includeCreatorCoinPolicyGrant = usesCreatorCoinPolicyController(args.vaultKind)
  const minFirstDeposit = MIN_FIRST_DEPOSIT
  const minimumTotalIdle = (minFirstDeposit * DEFAULT_MIN_IDLE_PERCENT_BPS) / 10_000n
  ...
  const floorPriceQ96ForBatcher = await (async () => {
    try {
      const quote = await computeMarketFloorQuote({
        publicClient: publicClient as Parameters<typeof computeMarketFloorQuote>[0]['publicClient'],
  ...
  const phase1Onchain = await readDeployedPhase1CoreAddresses({
    publicClient: publicClient as Parameters<typeof readDeployedPhase1CoreAddresses>[0]['publicClient'],
    batcherAddress: DEPLOYMENT_BATCHER,
  ...
  const phase1Any = phase1State.vaultDeployed || phase1State.wrapperDeployed || phase1State.shareOftDeployed
  const phase1All = phase1State.vaultDeployed && phase1State.wrapperDeployed && phase1State.shareOftDeployed
  if (phase1State.shareOftDeployed && (!phase1State.vaultDeployed || !phase1State.wrapperDeployed)) {
    throw new Error(
  ...
  const phase1SaltOverride = (vanityPlan.shareOftSaltOverrideUsed ?? ZERO_BYTES32) as Hex
  const asBatcherCall = (data: Hex) => ({ target: DEPLOYMENT_BATCHER, value: 0n, data }) as const
  ...
  const burnStreamAlreadyDeployed = Boolean(
    (await publicClient.getBytecode({ address: expectedBurnStream }).catch(() => null)) &&
      (await publicClient.getBytecode({ address: expectedBurnStream }).catch(() => null)) !== '0x',
  )
  const payoutRouterAlreadyDeployed = Boolean(
    (await publicClient.getBytecode({ address: expectedPayoutRouter }).catch(() => null)) &&
      (await publicClient.getBytecode({ address: expectedPayoutRouter }).catch(() => null)) !== '0x',
  )
  const creatorCoinPolicyControllerAlreadyDeployed = includePolicyController
    ? Boolean(
        (await publicClient.getBytecode({ address: expectedCreatorCoinPolicyController }).catch(() => null)) &&
          (await publicClient.getBytecode({ address: expectedCreatorCoinPolicyController }).catch(() => null)) !== '0x',
      )
    : true
        ...
        data: encodeFunctionData({
          ...
          args: [
            ...
            },
            {
              ...
              revenueRouter: resolveDeployLanePayoutRouterCodeId(args.vaultKind),
              revenuePolicyController: resolveDeployLaneRevenuePolicyControllerCodeId(args.vaultKind),
            },
          ],
        }),
      } as const)
  ...
  const batcherWhitelistPayoutRouterCall =
    wrapperOwnerAddress && sameAddress(wrapperOwnerAddress, DEPLOYMENT_BATCHER) && !payoutRouterWrapperWhitelisted
      ? ({
  ...
  const batcherSetPayoutRouterShareOftNoFeesCall =
    shareOftOwnerAddress && sameAddress(shareOftOwnerAddress, DEPLOYMENT_BATCHER) && !payoutRouterShareOftNoFees
      ? ({
          target: DEPLOYMENT_BATCHER,
          value: 0n,
          data: encodeFunctionData({
  ...
  const currentVaultBurnStream = await (async () => {
    try {
      ...
      return typeof value === 'string' && isAddress(value) ? getAddress(value as Address) : ZERO_ADDRESS
    } catch {
      return ZERO_ADDRESS
    }
  })()
  const burnStreamAlreadyConfigured = !sameAddress(currentVaultBurnStream, ZERO_ADDRESS)
  if (burnStreamAlreadyConfigured && !sameAddress(currentVaultBurnStream, expectedBurnStream)) {
    throw new Error(
  ...
  const configuredImpairmentGuardian = normalizeAddressLike(contracts.impairmentGuardian ?? null)
  if (configuredImpairmentGuardian && !sameAddress(currentVaultImpairmentConfig.guardian, configuredImpairmentGuardian)) {
  ...
  const impairmentChallengeWindowSecondsRaw = String(process.env.IMPAIRMENT_CHALLENGE_WINDOW_SECONDS ?? '').trim()
  const configuredImpairmentChallengeWindowSeconds = impairmentChallengeWindowSecondsRaw
  ...
  const impairmentAuxPlan = buildImpairmentAuxPlan({ vault: expected.vault, initialOwner: OWNER_CSW })
  ...
  const payoutMismatch =
    currentPayoutRecipient != null && !sameAddress(currentPayoutRecipient, expectedPayoutRouter)
  ...
  const phase2CoreState = await (async () => {
    ...
    const codes = await Promise.all(addrs.map((addr) => publicClient.getBytecode({ address: addr }).catch(() => null)))
  ...
  const phase2CoreAny = phase2CoreState.gaugeDeployed || phase2CoreState.ccaDeployed || phase2CoreState.oracleDeployed
  const phase2CoreAll = phase2CoreState.gaugeDeployed && phase2CoreState.ccaDeployed && phase2CoreState.oracleDeployed
  ...
  const attachedFinalizeCalls = await attachFinalizeShareBridgeValueToCalls({
    publicClient: publicClient as Parameters<typeof attachFinalizeShareBridgeValueToCalls>[0]['publicClient'],
    calls: [{ to: DEPLOYMENT_BATCHER, value: '0', data: finalizePhase2Calldata }],
  })
  ...
  ].filter((call): call is { target: Address; value: bigint; data: Hex } => call != null)

  const phase2ConfigCalls: Array<{ target: Address; value: bigint; data: Hex }> = []
  ...
  if (includeCreatorCoinPolicyGrant && creatorCoinPolicyControllerOwnershipPlan.needsGrant) {
  ...
  const marketV3InitialSqrtPriceX96 = await (async () => {
    try {
      ...
      const chainlink = getAddress(contracts.chainlinkEthUsd ?? BASE_DEFAULTS.chainlinkEthUsd)
  ...
  if (!ajnaKeeper) {
    throw new Error('Protocol Ajna keeper is not configured. Set PROTOCOL_AJNA_KEEPER before building the plan.')
  }
        ...
        args: [
          {
            ...
            charmVaultName: token.symbol ? `4626: ${token.symbol.toLowerCase()}/USDC` : '4626: CREATOR/USDC',
            charmVaultSymbol: token.symbol ? `CV-${token.symbol.toLowerCase()}-USDC` : 'CV-CREATOR-USDC',
            ajnaVaultName: token.symbol ? `Ajna 4626: ${token.symbol.toLowerCase()}/USDC` : 'Ajna 4626: CREATOR/USDC',
            ajnaVaultSymbol: token.symbol ? `AJ-${token.symbol.toLowerCase()}-USDC` : 'AJ-CREATOR-USDC',
          ...
          {
            charmAlphaVaultDeploy: keccak256(toHex('charm-factory-sentinel-v1')),
  ...
  const sessionPlan: SessionCreateRequest = {
    ...
    ...(phase3Calls.length > 0 ? { phase3Calls: serializeSessionCalls(phase3Calls) } : {}),
    ...(phase4Calls.length > 0 ? { phase4Calls: serializeSessionCalls(phase4Calls) } : {}),
    solanaOvault: { enabled: DEFAULT_SOLANA_OVAULT_MESH_ENABLED },
    vanity: {
