#!/usr/bin/env node
/**
 * End-to-end v1.14.0 greenfield deploy versioning checks:
 * - repo defaults vs live batcher.phase1Module()
 * - Phase1Module immutables (create2/store/modules) vs defaults
 * - moduleStorageVersion fingerprint (v3) vs deploy bytecode
...
function manifestPath(): string {
  ...
  return resolve(import.meta.dirname, '../../../deployments/base/v1.14.0-bytecode-manifest.json')
}
...
async function main(): Promise<void> {
  ...
  checks.push({
    ...
    ok: livePhase1 !== getAddress(PRE_V1140_SPLIT_PHASE1_PHASE1_MODULE),
    detail: `live=${livePhase1} retired_pre_v1140=${PRE_V1140_SPLIT_PHASE1_PHASE1_MODULE}`,
  })
  ...
  checks.push({
    id: 'deploy_bytecode.v3_fingerprint',
  ...
  checks.push({
    ...
    detail: localVaultBytecode.includes(V2_FINGERPRINT_HEX)
      ? 'DEPLOY_BYTECODE.CreatorOVault still embeds v2 fingerprint'
      : 'no v2 fingerprint in deploy bytecode',
  })
  ...
  if (pointerOk) {
    ...
    checks.push({
      id: 'store.creator_ovault_v3_fingerprint',
      ok: storeBytecode.includes(V3_FINGERPRINT_HEX),
      detail: storeBytecode.includes(V3_FINGERPRINT_HEX)
        ? `pointer=${pointer}`
        : `pointer=${pointer} missing v3 fingerprint`,
    })
  }
  ...
  process.stdout.write(`${JSON.stringify({ release: 'v1.14.0', ready, checks }, null, 2)}\n`)
