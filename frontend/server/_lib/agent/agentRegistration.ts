...
import { readProtocolCswAddressEnv } from '../wallet/canonicalCswEnv.js'
...
const CANONICAL_AGENT_WALLET_CAIP10 = `eip155:8453:${PROTOCOL_CSW_ADDRESS}` as const
...
const fallbackRegistration: RegistrationFile = {
  ...
  services: [
    ...
    {
      name: 'XMTP',
      endpoint: `https://xmtp.chat/dm/${PROTOCOL_CSW_ADDRESS}`,
      version: 'production',
      address: PROTOCOL_CSW_ADDRESS,
...
export function buildAgentRegistration(origin: string): {
  ...
  // ---------------------------------------------------------------------------
  // If PROTOCOL_CSW_ADDRESS is set, ensure XMTP / agentWallet reflect the protocol CSW.
  const cswAddress = readProtocolCswAddressEnv() || PROTOCOL_CSW_ADDRESS
