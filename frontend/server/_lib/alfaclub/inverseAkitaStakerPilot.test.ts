...
describe('inverseAkitaStakerPilot', () => {
  ...
  it('formats concise /h rules with access, commands, and playbook', () => {
    ...
    expect(stakerText).toContain('ticker + directional lean')
    expect(stakerText).toContain('mirrors the inverse')
    expect(stakerText).not.toContain('long btc')
    expect(stakerText).not.toContain('should i short btc')
    expect(stakerText).not.toMatch(/\bDrop any market take\b/)
