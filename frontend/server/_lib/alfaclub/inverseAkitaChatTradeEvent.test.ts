...
describe('parseInverseAkitaChatTradeEvent', () => {
  ...
  it('recognizes trade-completed sender', () => {
    expect(isAlfaClubTradeCompletedSender('trade-completed')).toBe(true)
    expect(isAlfaClubTradeCompletedSender('0xabc')).toBe(false)
  })
})

describe('collectInverseAkitaChatTradeIntents + trade-completed', () => {
  it('attributes HL opens to the room creator when payload has no wallet', async () => {
    vi.resetModules()
