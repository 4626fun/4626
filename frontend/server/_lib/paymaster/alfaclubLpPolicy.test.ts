...
async function validate(batch: AlfaClubLpInnerCall[], overrides: Record<string, unknown> = {}) {
...
describe('AlfaClub Sudoswap paymaster policy', () => {
  ...
  it('rejects a Room 1659 pair whose trading fee drifts from 690 bps', async () => {
    await expect(validate(routerOnly(), { fee: 6_900_000_000_000_000n })).rejects.toThrow(
  ...
  it('rejects stale, excessive, and zero deadlines', async () => {
    ...
    await expect(validate(routerOnly({ deadline: NOW + 1_201n }))).rejects.toThrow(
      'alfaclub_sudoswap_deadline_not_allowed',
    )
  })
  ...
  it('rejects a non-OK curve quote and a disabled adapter market', async () => {
    ...
    await expect(
      validate(routerOnly(), { markets: [ROOM_1659_CREATOR_COIN, 1659n, false] }),
  ...
  it('rejects approvals to any spender other than Permit2 and the adapter', async () => {
    ...
    await expect(validate(batch)).rejects.toThrow('alfaclub_sudoswap_erc20_approval_mismatch')
  })
