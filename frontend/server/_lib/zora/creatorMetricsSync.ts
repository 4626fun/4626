...
export async function ensureCreatorMetricsSchema(db: Db): Promise<void> {
  ...
  creatorMetricsSchemaEnsurePromise = (async () => {
    ...
    if (!tablesExist) {
      // Strongly prefer the authoritative migration.
      await ensureCreatorMetricsBaseSchema(db)
      // The four base tables (creator_coins, creators, creator_metrics_state, daily_snapshots)
      // are now defined only in the migration 20260527010000_creator_metrics_base_tables.sql.
      // Raw fallback blocks removed.
    }
