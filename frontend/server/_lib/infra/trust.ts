...
export function getTrustedRequestOrigins(req?: VercelRequest): Set<string> {
  ...
  // Waitlist join + returning-wallet bridge run on the marketing host and mint
  // `cv_auth_session` via POST /api/auth/privy. Dropping these origins 403s
  // cookie-bearing auth from https://4626.fun (exact client error: Forbidden).
