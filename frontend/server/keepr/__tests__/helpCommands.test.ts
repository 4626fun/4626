...
describe('keepr help commands', () => {
  ...
  it('returns full help with /help all', async () => {
    ...
    expect(result.response).toContain('Tip: use /help for quick mode, or /help <topic> for focused help.')
    expect(result.response).toContain('- /coin create <name> <symbol> <uri> — create Content Coin')
    expect(result.response).toContain('↳ name: 1-24 chars, symbol: 2-6 chars, url: https://...')
    expect(result.response).toContain('- /coin trend funnel <ticker> eth-amount — run guarded flywheel action')
    expect(result.response).toContain('- /mkt news <symbol> limit — recent headlines')
    expect(result.response).toContain('- /cre tend vault — deploy idle funds')
    expect(result.response).toContain('- /bankr balances base,solana')
    expect(result.response).toContain('- /reputation agentId — ERC-8004 reputation graph')
