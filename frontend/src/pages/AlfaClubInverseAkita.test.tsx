...
describe('AlfaClubInverseAkita', () => {
  it('explains the strategy identity, operating loop, and public surfaces', () => {
    ...
    expect(screen.getByAltText('AlfaClub logo').getAttribute('src')).toBe(
      '/protocols/alfaclub.svg',
    )
    expect(screen.getByAltText('Virtuals Arena logo').getAttribute('src')).toBe(
      '/protocols/virtuals.svg',
    )
    expect(screen.getByAltText('Hyperliquid logo').getAttribute('src')).toContain(
      'assets.coingecko.com',
    )
    expect(screen.getByAltText('Cabals logo').getAttribute('src')).toBe('/protocols/cabals.svg')
