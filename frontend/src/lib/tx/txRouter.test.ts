...
vi.mock('@/lib/base/baseBuilderCodes', async () => {
  const actual = await vi.importActual<typeof import('@/lib/base/baseBuilderCodes')>('@/lib/base/baseBuilderCodes')
  return {
    ...actual,
...
describe('txRouter', () => {
  ...
  it('sends generic multi-call batches sequentially for EOA execution', async () => {
    const sendTransaction = vi
      .fn()
      .mockResolvedValueOnce(HASH_A)
      .mockResolvedValueOnce(HASH_B)
    ...
    const result = await buildAndSendCalls({
      context,
      calls: [
        { to: ADDRESS_C, from: ADDRESS_B, data: '0xaaaa', value: '0', chainId: 8453 },
        { to: ADDRESS_A, from: ADDRESS_B, data: '0xbbbb', value: '0', chainId: 8453 },
      ],
    })
  ...
  it('submits generic multi-call batches through wallet_sendCalls when available', async () => {
    const request = vi.fn(async ({ method }: { method: string }) => {
      if (method === 'wallet_sendCalls') return '0xcallbundle'
      if (method === 'wallet_getCallsStatus') return { status: 200, receipts: [{ transactionHash: HASH_A }] }
      throw new Error(`unexpected method: ${method}`)
    })
    const context = makeContext({
      executionMode: 'canonical',
