...
describe('evaluateCanonicalSignerGate', () => {
  ...
  it('is ready for sub-account track when app wallet owner check passes', () => {
    const result = evaluateCanonicalSignerGate({
      executionMode: 'canonical',
  ...
  it('blocks sub-account track when provider is ready but app wallet owner is missing', () => {
    const result = evaluateCanonicalSignerGate({
      executionMode: 'canonical',
      executionTrack: 'sub-account',
      canonicalAddress: '0xab6d5c10b03300326cd7fab7267ae192842967b5',
      ...
      embeddedWalletCanSign: true,
      ownerCheckStatus: 'not-owner',
      subAccountOwnerCheckStatus: 'not-owner',
    })
  ...
  it('waits for app wallet owner check on counterfactual sub-accounts', () => {
    const result = evaluateCanonicalSignerGate({
      executionMode: 'canonical',
      ...
      embeddedWalletCanSign: true,
      ownerCheckStatus: 'not-owner',
      subAccountOwnerCheckStatus: 'unknown',
    })
  ...
  it('requires Base Account provider after app wallet owner is confirmed', () => {
    const result = evaluateCanonicalSignerGate({
      executionMode: 'canonical',
      executionTrack: 'migration-pending',
      canonicalAddress: '0xab6d5c10b03300326cd7fab7267ae192842967b5',
    ...
    expect(result.required).toBe(true)
    expect(result.ready).toBe(false)
    expect(result.code).toBe('base-sub-account-provider-missing')
  })

  it('does not fall back to parent owner on sub-account track', () => {
    const result = evaluateCanonicalSignerGate({
      executionMode: 'canonical',
      executionTrack: 'migration-pending',
      canonicalAddress: '0xab6d5c10b03300326cd7fab7267ae192842967b5',
