...
export function mapAddOwnerFundingErrorMessage(
  ...
  if (context?.fundingPreflightOk) {
    if (context.surface === 'add-owner-page') {
      return (
        'Base App refused to build the direct add-owner UserOp even though your CSW already has gas prefund. ' +
        'The Review sheet often shows a misleading "not enough funds" message — this is not RelayRouter multicall ' +
        '(this page submits CSW → CSW addOwnerAddress for EntryPoint handleOps). Tap Cancel on that sheet, then use ' +
        '"Submit via Relay Method A" below — that matches your earlier successful installs (Depository Part 1 + EntryPoint Part 2).'
      )
    }
    return (
