...
export async function resyncWaitlistGroupMembership(): Promise<{
  ...
  try {
    ...
    if (!response.ok || !json?.success) {
      return {
        ok: false,
        error: formatWaitlistChatError(json?.data?.error ?? json?.error ?? `waitlist_xmtp_resync_${response.status}`),
      }
    }
    ...
    return {
      ok: false,
      error: formatWaitlistChatError(err instanceof Error ? err.message : String(err)),
    }
  }
}
