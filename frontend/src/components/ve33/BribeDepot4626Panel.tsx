/**
 * BribeDepot4626 marketplace MVP — deposit current-epoch bribes and claim past epochs.
 * Vault address is the gauge id (same convention as ve4626GaugeVoting).
 */
import { useEffect, useMemo, useState } from 'react'
import { AnimatePresence, motion } from 'framer-motion'
import { formatUnits, isAddress, parseUnits, type Address } from 'viem'
import { useBribes4626 } from '@/hooks/useBribes4626'
...
export interface BribeVaultOption {
  address: string
  name: string
}

interface BribeDepot4626PanelProps {
  vaults?: BribeVaultOption[]
  className?: string
  /** Optional default bribe token (e.g. USDC / ShareOFT) */
  defaultToken?: string
}

export function BribeDepot4626Panel({
  vaults = [],
  className = '',
  defaultToken = '',
}: BribeDepot4626PanelProps) {
  const factoryAddress = CONTRACTS.bribesFactory4626
  ...
  const needsApproval = depot && amountWei > 0n && tokenAllowance < amountWei

  if (!factoryAddress || !votingAddress) {
    return (
      <div className={`bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 ${className}`}>
        <h3 className="text-xl font-bold text-brand-primary mb-4">Bribes</h3>
        <div className="text-center py-8">
          <div className="text-zinc-500 mb-2">Coming Soon</div>
          <p className="text-sm text-zinc-400">
            Deposit multi-token bribes for vault gauges. Voters claim pro-rata after the epoch ends.
          </p>
        </div>
      </div>
    )
  ...
  const handleEnsureDepot = () => {
    try {
      ensureDepot()
    } catch (err) {
      logger.error('ensureDepot failed', err)
    }
  }
  ...
  const handleDeposit = () => {
    try {
      depositBribe(amountWei)
    } catch (err) {
      logger.error('bribe deposit failed', err)
    }
  }
  ...
  const handleClaim = () => {
    ...
    try {
      claimBribe(claimEpoch, token)
    } catch (err) {
      logger.error('bribe claim failed', err)
    }
  }

  return (
    <div className={`bg-zinc-900/50 border border-zinc-800 rounded-2xl overflow-hidden ${className}`}>
      <div className="bg-linear-to-r from-brand-accent/10 to-brand-primary/10 border-b border-zinc-800 p-6">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            <h3 className="text-xl font-bold text-brand-primary">Bribe marketplace</h3>
            <p className="text-sm text-zinc-400 mt-1">
              Fund the current epoch · claim after it ends · pro-rata by gauge votes
            </p>
          </div>
      ...
      <div className="p-6 space-y-4">
        ...
        <label className="block">
          <span className="text-sm text-zinc-400 mb-1 block">
            Bribe token{tokenSymbol ? ` (${tokenSymbol})` : ''}
          </span>
          <input
            type="text"
            value={tokenInput}
          ...
          <div className="text-xs text-zinc-500 flex flex-wrap gap-3">
            <span>
              ...
              <span className="font-mono text-zinc-400">
                {depot ? shortAddress(depot) : 'not created'}
              </span>
            </span>
            <span>
              Bribe-eligible:{' '}
              <span className={canReceiveBribes ? 'text-green-400' : 'text-amber-400'}>
                {canReceiveBribes ? 'yes' : 'no'}
              </span>
            </span>
          </div>
        )}
          ...
          <>
            <label className="block">
              ...
              {token && (
                <div className="mt-1 text-xs text-zinc-500">
                  Balance:{' '}
                  {formatUnits(tokenBalance ?? 0n, tokenDecimals ?? 18)}
                  {tokenSymbol ? ` ${tokenSymbol}` : ''}
                  {depot && currentEpoch !== undefined && (
                    <>
                      {' · '}
                      Epoch bag:{' '}
                      {formatUnits(currentEpochTotalBribes ?? 0n, tokenDecimals ?? 18)}
                    </>
                  )}
                </div>
              )}
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              {!depot && (
                <button
                  type="button"
                  onClick={handleEnsureDepot}
                  disabled={isPending || !vault || !canReceiveBribes}
                ...
                >
                  {isPending ? 'Confirm in wallet…' : 'Create bribe depot'}
                </button>
              )}
              {depot && needsApproval && (
                <button
              ...
              {depot && !needsApproval && (
                <button
                  ...
                  disabled={
                    isPending || amountWei <= 0n || !token || !canReceiveBribes || amountWei > tokenBalance
                  }
                ...
                >
                  {isPending ? 'Depositing…' : 'Deposit bribe'}
                </button>
              )}
            </div>
          </>
        ...
        {tab === 'claim' && (
          <>
            ...
            <div className="bg-zinc-800/50 rounded-xl p-4 text-sm space-y-2">
              <div className="flex justify-between text-zinc-400">
                ...
                <span className="font-mono text-zinc-200">
                  {formatUnits(epochTotalBribes ?? 0n, tokenDecimals ?? 18)}
                  {tokenSymbol ? ` ${tokenSymbol}` : ''}
                </span>
              </div>
              <div className="flex justify-between text-zinc-400">
            ...
            >
              {isPending ? 'Claiming…' : 'Claim bribe'}
            </button>
          </>
        )}
      </div>
...
export default BribeDepot4626Panel
