/**
 * ve4626 gauge voting UI (canonical lowercase **ve** — never `Ve4626…`).
 * For JSX: `import { Ve4626GaugeVotingPanel as GaugeVotingPanel } from '...'`
 */
import { useState, useMemo } from 'react'
import { motion, AnimatePresence } from 'framer-motion'
import { useVe4626GaugeVoting, useTimeRemaining, formatVotingPower } from '../../hooks/useVe4626GaugeVoting'
...
interface Ve4626GaugeVotingPanelProps {
  vaults?: VaultInfo[]
  className?: string
}

export function Ve4626GaugeVotingPanel({ vaults = [], className = '' }: Ve4626GaugeVotingPanelProps) {
  const {
    epochInfo,
    ...
    isVoting,
    txSuccess,
  } = useVe4626GaugeVoting({
    votingAddress: CONTRACTS.ve4626GaugeVoting,
  ...
  if (!CONTRACTS.ve4626GaugeVoting || !CONTRACTS.ve4626) {
    return (
...
export function Ve4626GaugeVotingMini({ className = '' }: { className?: string }) {
  const { epochInfo, votingPowerInfo, hasVotedThisEpoch } = useVe4626GaugeVoting({
    votingAddress: CONTRACTS.ve4626GaugeVoting,
    ve4626Address: CONTRACTS.ve4626,
  })
  ...
  if (!CONTRACTS.ve4626GaugeVoting) {
    return null
  }
...
export default Ve4626GaugeVotingPanel
