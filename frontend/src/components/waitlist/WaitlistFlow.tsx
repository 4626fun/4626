...
import { useAccount, usePublicClient, useSignMessage, useSwitchChain, useWalletClient } from 'wagmi'
...
export function WaitlistFlow(props: { variant?: Variant; sectionId?: string }) {
  ...
  const ownerInstallStatus = useMemo(
    () =>
      deriveOwnerInstallMappingStatus({
        ...
        ownerInstallBusy: embeddedOwnerInstallBusy,
      }),
    [
  ...
  const canSubmit =
    Boolean(verifiedWalletNormalized) &&
    emailOk &&
    (Boolean(creatorCoin?.address) || creatorCoinDeclaredMissing) &&
    connectedWalletAuthorized &&
    (!effectiveCswAddress || ownerInstallStatus === 'READY_FOR_OWNER_INSTALL')
  ...
  const ownerInstallGate = useMemo(() => {
    switch (ownerInstallStatus) {
      ...
      case 'OWNER_INSTALL_CHECKING':
        return {
          ctaLabel: null,
          helper: 'Checking embedded owner access on your smart wallet...',
          onAction: null,
          busy: true,
        } as const
      ...
      case 'OWNER_INSTALLING':
        return {
          ctaLabel: null,
          helper: 'Confirm the owner-install transaction in your wallet...',
          onAction: null,
          busy: true,
        } as const
      ...
      default:
        return {
          ctaLabel: null,
          helper: 'Wallet mapping complete. Embedded owner is installed.',
          onAction: null,
          busy: false,
        } as const
    }
    ...
    installEmbeddedOwnerOnCanonicalCsw,
    linkZoraReadOnly,
    openInAppPrivyLogin,
    ownerInstallStatus,
    ...
    try {
      ...
      if (deployUrl.startsWith('http')) {
        ...
        try {
          ...
          // Cross-origin handoff: mint one-time code server-side (never place session token in URL).
          if (typeof window !== 'undefined' && parsed.origin !== window.location.origin) {
            ...
            const handoffJson = handoffRes
              ? ((await handoffRes.json().catch(() => null)) as
                  | { success?: boolean; data?: { code?: string } | null }
                  | null)
              : null
            const handoffCode =
  ...
  }, [apiFetch, deployPath, deployUrl, getAccessToken, navigate, patchWaitlist, privyAuthed, siwe])
  ...
  const markAction = useCallback(
    (action: ActionKey) => {
      ...
      if (doneEmail && SERVER_CLAIMABLE_BONUS_TASKS.has(action)) {
        void (async () => {
          try {
