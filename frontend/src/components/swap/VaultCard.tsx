...
export function VaultCard({ vault, compact = false, withMyVault = false }: VaultCardProps) {
  ...
  const tvl = Number(data.totalAssets ?? 0n)
  const formattedTvl = tvl > 0 ? tvl.toLocaleString(undefined, { maximumFractionDigits: 2 }) : '—'
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="text-right text-xs text-zinc-500">
          <div>TVL</div>
          <div className="text-sm text-zinc-200">{compact ? `$${formattedTvl}` : `$${formattedTvl}`}</div>
        </div>
      </div>
