...
import { AlfaClubPoolManager } from "@/components/alfaclub/AlfaClubPoolManager";
...
function LpPositionCard(props: {
  ...
  const pair =
    [position.token0Symbol, position.token1Symbol]
      .filter(Boolean)
      .join(" / ") || "Unknown pair";
  const feeTier = position.feeTier
    ? `${(Number(position.feeTier) / 10000).toFixed(2)}%`
    : "--";
  const range =
    position.tickLower !== undefined && position.tickUpper !== undefined
      ...
      : "--";
  const fees =
    [position.tokensOwed0, position.tokensOwed1].filter(Boolean).join(" / ") ||
    "--";
...
export function LiquidityPanel(props: {
  ...
  const handleLpQuote = async () => {
    ...
    try {
      const t0 =
        props.tokenIn.trim().toLowerCase() === NATIVE_TOKEN_ADDRESS
          ? CONTRACTS.weth
          : props.tokenIn;
      const t1 =
        props.tokenOut.trim().toLowerCase() === NATIVE_TOKEN_ADDRESS
          ? CONTRACTS.weth
          : props.tokenOut;
      await quoteCreatePosition({
        ...
        lowerTick:
          lpMode === "advanced" && lpLowerTick.trim()
            ? Number(lpLowerTick)
            : undefined,
        upperTick:
          lpMode === "advanced" && lpUpperTick.trim()
            ? Number(lpUpperTick)
            : undefined,
      });
  ...
  const handleCreatePosition = async () => {
    ...
    try {
      ...
      setLpStatus(
        `Position submitted${(data as Record<string, unknown>)?.requestId ? ` (#${(data as Record<string, unknown>).requestId})` : ""}`,
      );
    } catch (e: unknown) {
  ...
  const lpPositionsQuery = useQuery({
    ...
    enabled: Boolean(
      props.activePanel === "liquidity" && props.canonicalAddress,
    ),
    queryFn: async () =>
      fetchLiquidityPositions(props.canonicalAddress!, BASE_CHAIN_ID),
    refetchInterval:
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          >
            ...
            <path d="M8 2v1M8 13v1M2 8H1m13 0h1M4.05 4.05l-.71-.71m9.32 9.32-.71-.71M4.05 11.95l-.71.71m9.32-9.32-.71.71" />
          </svg>
        </button>
      </div>

      <div
        className="grid grid-cols-2 gap-2 rounded-2xl border border-white/[0.08] bg-black/25 p-1.5"
        aria-label="Liquidity protocol"
      >
        <button
          type="button"
          onClick={() => setLiquidityProtocol("alfaclub")}
          className={`min-h-10 rounded-xl px-3 text-sm font-semibold transition ${
            liquidityProtocol === "alfaclub"
        ...
        <button
          type="button"
          onClick={() => setLiquidityProtocol("uniswap")}
          className={`min-h-10 rounded-xl px-3 text-sm font-semibold transition ${
            liquidityProtocol === "uniswap"
          ...
          <div className="rounded-2xl border border-white/8 bg-vault-card/50 p-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() =>
                  setLpMode(lpMode === "simple" ? "advanced" : "simple")
                }
            ...
            <div className="mt-2 grid grid-cols-2 gap-2">
              <div>
                <label htmlFor="swap-lp-fee-tier" className="label mb-1 block">
                  Fee tier
                </label>
                <select
                  id="swap-lp-fee-tier"
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              ...
              <button
                ...
                disabled={
                  anyBusy || !props.identityReady || !lpPositionId.trim()
                }
            ...
            {lpStatus && (
              <div className="mt-2">
                <Alert variant="success">{lpStatus}</Alert>
              </div>
            )}
            {lpError && (
              <div className="mt-2">
                <Alert variant="error">{lpError}</Alert>
              </div>
            )}
          </div>
            ...
            {lpPositionsQuery.isLoading && (
              <div className="space-y-2">
                {[1, 2].map((i) => (
                  <div
                    key={i}
                    className="h-24 animate-pulse rounded-2xl bg-white/4"
                  />
                ))}
              </div>
            )}
