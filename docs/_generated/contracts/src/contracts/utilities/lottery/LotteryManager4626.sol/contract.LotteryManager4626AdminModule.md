...
[Git Source](https://github.com/wenakita/4626/blob/2951e17122326ff4a23b28e80356c44121ebf59c/contracts/utilities/lottery/LotteryManager4626.sol)
...
```solidity
mapping(address => CreatorStats) public tokenStats
```
...
Legacy single-call setter for `ve4626GaugeVoting`.
...
After this is called, `setBoostManager` / `setve4626GaugeVoting`
...
### proposeve4626GaugeVoting

Propose a new `ve4626GaugeVoting`. Symmetric to `proposeBoostManager`.
...
### commitve4626GaugeVoting

Commit a previously proposed `ve4626GaugeVoting` once the timelock has elapsed.
...
### cancelve4626GaugeVotingProposal

Cancel an in-flight `ve4626GaugeVoting` proposal during the timelock window.
...
|`pendingGauge`|`address`|The pending replacement for `ve4626GaugeVoting`, or address(0).|
|`gaugeEffectiveAt`|`uint256`|Timestamp at which `commitve4626GaugeVoting` may run, or 0 if no proposal.|
