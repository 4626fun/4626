[**4626-app**](../../index.md)

***

[4626-app](../../index.md) / server/commands/execute

# server/commands/execute

## Functions

### executeCommand()

> **executeCommand**(`params`): `Promise`\<[`KeeprCommandResult`](types.md#keeprcommandresult)\>

Defined in: [server/commands/execute.ts:31](https://github.com/wenakita/4626/blob/main/frontend/server/commands/execute.ts#L31)

#### Parameters

##### params

[`ExecuteCommandParams`](types.md#executecommandparams)

#### Returns

`Promise`\<[`KeeprCommandResult`](types.md#keeprcommandresult)\>
