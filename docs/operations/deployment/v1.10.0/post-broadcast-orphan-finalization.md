...
commands come from docs/operations/deployment/releases/v1.10.0-vercel-env-plan.md,
...
Context
The v1.10.0 broadcast has completed. As of this prompt's run:
...
in docs/operations/deployment/releases/v1.10.0-mainnet.md.
...
The full 6-orphan list (from cursor-deploy-prompt-v1.10.0-cleanup.md §0):
...
This prompt runs against a fresh branch chore/v1.10.0-orphan-finalization,
branched from current main (which now contains the v1.10.0 broadcast
evidence merge).

Do NOT modify docs/operations/deployment/releases/v1.10.0-mainnet.md
...
release/v1.10.0-broadcast-evidence PR.

cat docs/operations/deployment/releases/v1.10.0-mainnet.md | head -60
...
tools/ci/check_manager_amoe_surface.sh <NEW_V1.10.0_MANAGER> https://mainnet.base.org
exits 0 with all 3 selectors found. Capture the new manager
address from v1.10.0-mainnet.md — call it $NEW_MANAGER for
...
git checkout -b chore/v1.10.0-orphan-finalization

CHECKPOINT 0 — pre-flight signoff
Print the result of all 8 checks. Wait for "go".

§2. On-chain balance sweep
Read-only. For each of the 6 orphan addresses + the new v1.10.0 manager
...
do
  ...
  printf '%-44s %s wei\n' "$addr" "$bal"
done | tee /tmp/v1.10.0-orphan-balances.txt
...
Save /tmp/v1.10.0-orphan-balances.txt for inclusion in the
follow-up PR body.
...
blocks from docs/operations/deployment/releases/v1.10.0-vercel-env-plan.md
...
the v1.10.0 target value

the exact vercel env rm and vercel env add commands

The expected vars are some subset of:

LOTTERY_AMOE_ROUTER (currently 0xC618…3759, target = new v1.10.0 router)
...
vercel deploy --prod --cwd <project-root> 2>&1 | tee /tmp/v1.10.0-postsweep-deploy.log
...
return 503 with the disabled errors. The whole point of v1.10.0 is
...
for path in submit-zk burn-credits publish-cron burn-refund-cron retry-cron; do
  ...
  cat /tmp/resp.json
  echo
done | tee /tmp/v1.10.0-postsweep-smoke.txt
(Adjust the URL paths to match what the evidence doc actually used —
the path schema may differ.)

Save /tmp/v1.10.0-postsweep-smoke.txt for the PR body.

text
<<< INSERT v1.10.0-vercel-env-plan.md COMMAND BLOCKS HERE >>>
...
blocks from docs/operations/deployment/releases/v1.10.0-supabase-update-plan.md
into the marked region below.**
...
0xC618Dde25F0085F3b2BC3a48ba806F8Fc9a93759 (replacement router)	NO	Owned by Safe 0x7d42…f2d3. Keeping Safe ownership preserves the option to sweep stuck ETH/tokens or pause-then-decommission cleanly. The Safe is the same one that owns the v1.10.0 router.
...
Note:     Pre-PR #395 build, missing AMOE selectors. Replaced by v1.10.0
          manager <NEW_MANAGER>. Do not interact. See:
          https://github.com/wenakita/4626/blob/main/docs/operations/deployment/releases/v1.10.0-mainnet.md
...
Note:     Pre-PR #395 build, missing AMOE selectors. Was target of
          replacement router 0xC618…3759. Replaced by v1.10.0 manager
...
Note:     Correctly Safe-owned but its target manager has no AMOE
          handler. Replaced by v1.10.0 router <NEW_ROUTER>. Do not
          interact. See: <link>
...
Note:     Companion to deprecated router 0xC618…3759. Replaced by
          v1.10.0 verifier <NEW_VERIFIER>. See: <link>
...
Note:     Owned by Privy custodial smart wallet whose execution path
          ...
          by v1.10.0 router <NEW_ROUTER>. See: <link>
...
<link> with the actual values from v1.10.0-mainnet.md.
...
Evidence (partial — has the v1.10.0 release notes link, may be
...
Commit to chore/v1.10.0-orphan-finalization:

text
chore(v1.10.0): orphan registry — fill in cleanup actions for 6 v1.8.3/v1.9.x orphans
CHECKPOINT 6 — registry diff review
§8. Retire test/v183-release-target-guard.sh
This script asserts the v1.8.3 epoch is canonical. Post-v1.10.0,
...
and exits 1 if absent. After v1.10.0, those v1.8.3 addresses
should NOT be in the "Current Live Infrastructure" section.

It greps Current Live Infrastructure (\v1.8.3`)` as a
required heading. After v1.10.0, that heading reads
Current Live Infrastructure (\v1.10.0`)`.
...
pinning for v1.10.0 — same hardcoded-list-with-whitelist pattern,
pointing at the v1.10.0 manager / router / verifier / 17 infra
...
leaking in"; this one says "the canonical v1.10.0 addresses
are still where we expect."

Commit to chore/v1.10.0-orphan-finalization:

text
chore(v1.10.0): retire v1.8.3 release-target guard, add v1.10.0 successor
CHECKPOINT 7 — guard retirement diff review
§9. Open the follow-up PR
Push chore/v1.10.0-orphan-finalization and open a PR to main.

PR title:
chore(v1.10.0): orphan finalization — Vercel/Supabase rotation, Basescan labels, registry fill-in, retire v1.8.3 guard

PR body must contain, in this order:

Summary — one paragraph: "Closes the v1.10.0 redeploy by
...
Vercel rotation — paste contents of /tmp/v1.10.0-postsweep-deploy.log
summary line + the full smoke-check output from §3.

Supabase rotation — paste contents of
/tmp/v1.10.0-supabase-rotation.txt.

Balance sweep — paste /tmp/v1.10.0-orphan-balances.txt.

Owner verification — paste /tmp/v1.10.0-orphan-owners.txt,
...
The v1.10.0 redeploy is complete. The next active work is:

48h safe-mode soak on the v1.10.0 stack with all AMOE_*_ENABLED
flags still off. Smoke-check daily.

§3.0.5 of amoe-flag-rollout-plan.md — the
manager.setAuthorizedAmoeRelayer(<v1.10.0 router>) one-way trust
handoff. Counsel-gated. Single Safe tx.

Phases 0.5 → 4 of amoe-flag-rollout-plan.md — the gradual
flag flip sequence in Vercel.

cursor-deploy-prompt-amoe.md rev 5 (updated by the main v1.10.0
