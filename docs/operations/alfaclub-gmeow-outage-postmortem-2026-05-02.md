...
| 6 | Write `docs/operations/alfaclub/token-rotation.md`: how to mint fresh Privy tokens from a real browser session, which JSON field maps to which env var, and the one-shot Vercel API curl that pushes all three to Production and Preview. | bridge | P0 - same day |
