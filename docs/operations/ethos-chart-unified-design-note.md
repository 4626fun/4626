...
- New "Suggested New Indexes (from access patterns + live slow queries)" + "Recent Expensive Chart Queries (Tagged)" sections surface slow queries from `pg_stat_statements` (via the `ethos_expensive_chart_queries` view + our `supabase-chart:*` application_name tagging). Recommendations are now **partially dynamic**: the handler inspects live expensive queries and reinforces or annotates suggestions with "derived from live slow queries" evidence when the observed patterns match (still strictly on the single source: creator_ethos_projection + v_explore_creators + snapshots).
