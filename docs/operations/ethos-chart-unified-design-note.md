...
- New "Suggested New Indexes (from access patterns + live slow queries)" + "Recent Expensive Chart Queries (Tagged)" sections surface slow queries from `pg_stat_statements` (via the `ethos_expensive_chart_queries` view + our `supabase-chart:*` application_name tagging). Recommendations are now **partially dynamic** and each carries a **"Copy full migration file"** button that emits a complete, timestamped Supabase migration (header with rationale + observed slow query tags, the `CREATE INDEX CONCURRENTLY`, and rollback comment). Everything remains strictly on the single source: creator_ethos_projection + v_explore_creators + snapshots. No CREATEs ever execute from the UI.
- Manual SQL fallbacks remain available. All operations stay on the single interconnected source tables/views. New index suggestions are strictly advisory.
