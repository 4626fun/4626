...
If hot cron (`ethos-sync-hot` every 2m on even minutes, excluding full-sync `:24`/`:54`) is healthy, set `KEEPER_ETHOS_SYNC_ENQUEUE_ENABLED=0` to drop the redundant keeper duplicate lane.
