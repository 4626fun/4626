# Vault deployment — technical runbook

Tier 2 archive. Load for Foundry/AA/batcher deploy paths, preflight, and troubleshooting.

Product invariants (pricing, strategy bundle, bytecode store): [deploy-cutovers-vault.md](./deploy-cutovers-vault.md).

Rule: `.cursor/rules/deploy-ops.mdc`. Validate: `pnpm -C frontend validate:deploy-guards` (scoped).
...
# 2) Legacy OVaultFactory4626 registrar is useful to see “which stack was registered by which legacy infra deploy”.
