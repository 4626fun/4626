...
This page explains every **active** wallet role used for one 4626 user and how those roles fit together.

## Why this architecture exists

4626 did not start with a four-role mental model. This setup emerged from balancing three constraints:
...
- Embedded signer authorizes parent-CSW sponsored UserOps without repeated passkey prompts — this is the default user execution path (`legacy-owner-install` / `canonical4337`).
...
Onboarding and deploy use the **parent CSW + Privy embedded-owner signer**. Sponsored swaps send from the parent CSW via `canonical4337`.
...
| <span style="background:#DCFCE7;color:#14532D;border:1px solid #16A34A;border-radius:999px;padding:2px 8px;font-size:12px;font-weight:600;">Signer</span><br/>Privy embedded EOA | Primary signer for parent-CSW sponsored UserOps | Reduce repeated passkey prompts for normal usage after owner install | Embedded key custody and owner status are reliable | Signer lane interruption; signer-role confusion in sessions/UI | Explicit signer status messaging; reconnect handling; keep custody vs signer copy clearly separated |
