---
name: 4626-solana-kpr
description: Solana and KPR operations skill for 4626. Use for Solana route provisioning, bridge/token setup, Meteora orchestration, and KPR keeper runtime changes with strict trust-boundary handling and standalone guidance when repository access is incomplete.
---
...
Use for Solana provisioning and KPR runtime tasks, including bridge route setup and cross-chain state relay operations.

Trigger when the user asks to modify route provisioning, token registration, bridge/Meteora setup, or KPR keeper behavior.
...
- **KPR role:** keeper workflows relay/settle/monitor between Solana and Base.
...
6. Validate relevant surfaces:
  ...
  - `cd kpr && tsx runner.ts` for keeper runtime verification
