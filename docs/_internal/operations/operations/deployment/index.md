---
title: Deployment
sidebar_position: 1
---

# Deployment Operations

Guides for deploying 4626 infrastructure on Base.

## Start here

| Guide | Description |
|-------|-------------|
| [Current release (v1.19.1)](/operations/deployment/releases/current) | Active greenfield deploy target |
| [Greenfield launch readiness](/operations/vault/greenfield-launch-readiness) | Pre-deploy readiness gate |
| [Deploy dry-run (local fork)](/operations/deployment/deploy-dry-run-local-fork-invariants) | Local Anvil fork invariants |
| [Pipe A batcher cutover](/operations/deployment/batcher-pipe-a-cutover) | Payable finalize + ShareOFT peer wiring |
| [Eliza runtime](/operations/deployment/eliza-runtime) | XMTP Railway primary |

## Checklists

| Guide | Description |
|-------|-------------|
| [Pre-launch](/operations/deployment/pre-launch) | Pre-launch checklist |
| [CCA verification](/operations/deployment/cca-verification) | Verify CCA deployment |
| [Approvals checklist](/operations/deployment/approvals-checklist) | Required approvals |
| [CREATE2 registry](/operations/deployment/create2-registry) | Deterministic deployments |
| [Infra epoch redeploy](/operations/deployment/infra-epoch-redeploy) | Registry/batcher/store rotation |

## Multisig

| Guide | Description |
|-------|-------------|
| [Multisig guide](/operations/deployment/multisig/guide) | Safe multisig usage |
| [Deploy with multisig](/operations/deployment/multisig/deployment) | Multisig deploy flow |
| [Owner setup](/operations/deployment/multisig/owner-setup) | Configure ownership |

Prior release packets (v1.7–v1.14.0) are in `docs/_internal/deployment-releases-legacy/` — repo-only.
