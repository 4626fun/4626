...
Important:
- **Index 0 is invalid** for `addQuoteToken` / `moveQuoteToken` (Ajna reverts `InvalidIndex`).
- The indices are **not centered at 3696**. The anchor for price \(=1.0\) is **4156**.
...
- Ajna **bucket price** = **CREATOR per 1 USDC** (i.e., inverse of “USDC per CREATOR”)

**Anchor**: Bucket **4156** corresponds to **price = 1.0** (1 CREATOR per 1 USDC).
- **Lower index (< 4156)** → **higher** price (more CREATOR per USDC)
- **Higher index (> 4156)** → **lower** price (less CREATOR per USDC)
...
## How to Calculate the Right Bucket (Recommended)
...
# If tick = 5000: bucket ≈ 4156 - (5000/50) = 4056
BUCKET=$((4156 - ($TICK / 50)))
```
...
Where:
- `index = 4156` → price = 1.0 (quote per collateral)
- `index = 4056` → price ≈ 1.005^(100) ≈ 1.65
- `index = 4256` → price ≈ 1.005^(-100) ≈ 0.61

### Examples

```
Index 1     → extremely high price (very expensive to borrow)
Index 4156  → price = 1.0
Index 7388  → very low price (very cheap to borrow)
```
