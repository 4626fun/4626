...
| ODA v2 **429–433** Batcher / Registry / Strategies / Gauge / ve4626 | pending (litterboxes still HTTP 200) |
...
| 3 | **Registry F1/F3/F4** from ODA 422 | One-shot / reverse-map guards | Complements SCAN-M3 |
| 4 | **Halmos CI** on `test/halmos/CreatorOVaultMath.t.sol` | `halmos --contract CreatorOVaultMath --function check_` with cached `out/` | Symbolic math already modeled; Foundry fuzz green |
| 5 | **Pashov `skills/solidity-auditor`** on lottery delta after 426 patches | In-repo skill | Depth pass without another $1 |
| 6 | **Mythril** on `_processWin` / jackpot payout / AMOE ZK submit | `myth analyze` or SolidSecs wrapper | Symbolic/bytecode on economic paths |
| 7 | **Re-`lake build` Aristotle** topics if Lean available | `docs/audits/aristotle/*` | Only needed when odds/boost formulas change |
| 8 | **CI `security-scanning.yml`** manual dispatch | GitHub Actions | Full Slither + Semgrep + gitleaks allowlist |
...
v1 jobs **418–421** (and any CreatorVault-scoped report) — wrong tree. Prefer v2 litterbox jobs and already-usable **422/423/426**.
