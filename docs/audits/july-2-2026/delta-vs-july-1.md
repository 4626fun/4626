...
| H-03 — Emergency vote reset → stale bribe over-claim | **VERIFIED FIXED** | `contracts/shared/governance/ve4626GaugeVoting.sol:434-437` — `getUserVoteWeightAtEpoch` returns 0 when `_userVoteGeneration[epoch][user] != _epochResetGeneration[epoch]`; generation bumped on reset (`:503`), stamped on vote (`:266-267`). |
