...
- **Observed behavior**: Both service hostnames resolve to Vercel and return SPA HTML; POST reconcile paths return 405; 7 of 15 production prelaunch gates fail from this root cause.
...
- **Pass/Fail criterion**: PASS when both health endpoints return service JSON (`ok: true` / `payerHealthy: true`) and `pnpm -C frontend ops:verify-akita-prelaunch --production` exits 0 with 0 blockers.
- **Minimal remediation recommendation**: Correct DNS A-records for `orchestrator.4626.fun` and `provisioner.4626.fun` to the Vultr hosts, not Vercel.
