...
| Decision | Choice |
|----------|--------|
| **Audit type** | **Combined**: (a) engineering hygiene and CI parity, (b) application/trust-boundary review aligned with [AGENTS.md](../../AGENTS.md), (c) protocol/economic pointers via existing [system.md](./system.md) (no new formal verification). |
...
| Forge build | `forge build --sizes` | **Exit 1** — some contracts exceed EIP-170 runtime size (expected; matches CI `|| true` non-blocking behavior in [.github/workflows/test.yml](../../.github/workflows/test.yml)). |
...
|---------------------------|-------------------------|---------------------------|---------------|
| Deploy status / preflight **read-only**; no infra mutation as side effect | Malicious or buggy deploy UI triggers unintended onchain/offchain writes | Deploy session: `preflightOnly` path and rate limits in [frontend/api/_handlers/deploy/session/_create.ts](../../frontend/api/_handlers/deploy/session/_create.ts); Solana preflight wiring in [_status.ts](../../frontend/api/_handlers/deploy/session/_status.ts), [_continue.ts](../../frontend/api/_handlers/deploy/session/_continue.ts). | Preflight still depends on correct client flags and handler branching; review any new deploy endpoints for the same split. |
| Internal Solana mutation requires **machine auth**; no ambient session fallback for privileged setup | Stolen user session used to provision routes | `registerSolanaBridgeToken`: [readDeployAuthFromRequest](../../frontend/server/_lib/deployAuth.ts) (session or SIWA agent); registration secret checks (`DEPLOY_SOLANA_REGISTRATION_SECRET`). CRE/keeper: `KEEPR_API_KEY` on [cre/keeper/_sweep.ts](../../frontend/api/_handlers/cre/keeper/_sweep.ts), [_markSettled.ts](../../frontend/api/_handlers/cre/keeper/_markSettled.ts), [_solanaReconcile.ts](../../frontend/api/_handlers/cre/keeper/_solanaReconcile.ts), [cre/vaults/_active.ts](../../frontend/api/_handlers/cre/vaults/_active.ts), [keepr/actions/_updateStatus.ts](../../frontend/api/_handlers/keepr/actions/_updateStatus.ts); status: [_solanaInfraStatus.ts](../../frontend/api/_handlers/deploy/_solanaInfraStatus.ts). | `KEEPR_API_KEY` secrecy is critical; rotate on leak; ensure no route registers tokens without aligned auth + secrets. |
| Telegram Mini App link completion requires **fresh session proof** | Forged or replayed Telegram context | [miniAppAuth.ts](../../frontend/api/_handlers/telegram/webhook/miniAppAuth.ts) (initData hash, replay keying); tests in [telegramMiniAppAuth.test.ts](../../frontend/api/__tests__/telegramMiniAppAuth.test.ts), [telegramEndpoints.test.ts](../../frontend/api/__tests__/telegramEndpoints.test.ts). | Depends on `TELEGRAM_BOT_TOKEN` and clock/skew policy; keep tests in sync with WebView behavior. |
