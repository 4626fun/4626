...
- Optional automation / CRE architecture
  - Multiple workflows can influence charm rebalance behavior: direct `charm-rebalance-manager`, event-driven enqueue (`strategy-event-listener` / `charmManager`), and queue executor (`keepr-queue-executor`).
