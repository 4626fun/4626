...
- **Hermit meme assets should publish through Pinata/IPFS for production.** Do not rely on local `frontend/public` assets as the durable production source; `https://ipfs.4626.fun` is the configured custom gateway, and public gateway reads do not require gateway keys or IP allowlists.
