...
- **SolanaBridgeAdapter has two generations on Base mainnet.** v1 at `0x90F578A4e23c1cB8DDFE63fd496ED7F4474f2b00` (historical) holds the pre-strict-parity AKITA mapping to `HuY4cQk5wJBfdaduUFnLJUiqhDXKyMR7mgSUHpZN9ouR` ("Zora Creator Coin"/"ZORA", wrapped Feb 2026). v2 at `0x653326dD0145656eC3b598943C0E84d7405aE6Ae` (deployed 2026-04-19 in tx `0xfe49c9e2...fd5e`) is the canonical default (`CONTRACTS.solanaBridgeAdapter`, `VITE_SOLANA_BRIDGE_ADAPTER`) and holds the lowercase-parity AKITA mapping to `9JWhbEAVpuHQdx1x5kSH62p6ZrWivqcBfARhvdLsLJdp` ("akita"/"akita"). Both adapters are plain `Ownable` (not upgradeable); migration strategy is "deploy fresh, swap the default" rather than in-place upgrade. `SolanaBridgeAdapter.registerToken` hard-reverts if `tokenToSolanaMint[baseToken] != 0`, so re-registering on v1 isn't possible — any future Solana-side re-brand requires yet another adapter deploy.
- **Solana naming invariant is canonical-documented and parity-verifiable.** `docs/operations/solana-bridge-naming-invariant.md` is the single source of truth for the lowercase-coerced Solana mint naming policy, PDA derivation formula, AKITA v1→v2 migration history, and per-drift runbook. Shared PDA derivation lives in `frontend/server/_lib/onchain/solanaWrappedMintPda.ts` (`deriveWrappedMintPda`) with golden fixtures pinned to live mainnet state (AKITA v1 `HuY4…9ouR` and v2 `9JWh…LJdp`). To check any creator's parity end-to-end (Base ERC-20 → adapter mapping → Solana on-chain metadata), run `pnpm -C frontend exec tsx scripts/verify-solana-mint-parity.ts --creator 0x<creator>` — exit 0 = parity, exit 2 = drift with a named reason. The programmatic entry point `verifyCreatorSolanaMintParity` in the same `_lib/onchain` folder returns a structured `{matched, drift[]}` result and is safe to call from scripts, CI, or status endpoints.
