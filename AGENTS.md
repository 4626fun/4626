...
- Rename user-facing labels that describe CSW owner-install flows to match the parent-CSW embedded-owner architecture (e.g. "Enable 4626 signing" rather than "Connect Coinbase Smart Wallet"), and do not make waitlist activation depend on explicit sub-account creation.
...
- **AlfaClub chat API calls may route through the optional `cloudflare-workers/alfaclub-proxy/` relay.** When the proxy is configured, `chatBridge` must keep the AlfaClub browser-fingerprint headers derived from the upstream API origin and send the proxy shared secret only to the relay.
- **AlfaClub websocket sends may route through the optional `fly-alfaclub-ws/` proxy.** `chatBridge` uses `ALFACLUB_WS_PROXY_URL`, `ALFACLUB_WS_PROXY_HTTP_SEND_URL`, and the matching proxy secret only when configured; the direct AlfaClub socket path remains the default when those env vars are unset.
...
- **DeployVault external signer buttons must use the clicked wallet connector directly.** Named Rabby / MetaMask / Coinbase buttons should request the provider/signer from that connector, switch that provider to Base, and avoid relying only on global wagmi connector state or a stale `useWalletClient` render tick.
