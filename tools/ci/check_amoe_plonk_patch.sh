...
#      `tools/zk/_patch_amoe_plonk_verifier.py` produces from the canonical
...
#
# Usage:
#   tools/ci/check_amoe_plonk_patch.sh
#
# Exit codes:
#   0  all checks pass
...
RAW="$ROOT/circuits/amoe/build/plonk_fresh/AmoePlonkVerifier_raw.sol"
...
if grep -qE '^contract\s+PlonkVerifier\b' "$VERIFIER"; then
  fail "AmoePlonkVerifier.sol still contains 'contract PlonkVerifier'. Run tools/zk/regen_amoe_plonk_verifier.sh."
fi
...
if [ -f "$RAW" ] && command -v python3 >/dev/null 2>&1; then
  TMP="$(mktemp)"
  trap 'rm -f "$TMP"' EXIT
  python3 "$ROOT/tools/zk/_patch_amoe_plonk_verifier.py" \
    --raw "$RAW" --out "$TMP"
  if ! diff -q "$VERIFIER" "$TMP" >/dev/null; then
    info "diff between committed and regenerated:"
    diff -u "$VERIFIER" "$TMP" | head -40 >&2 || true
    fail "AmoePlonkVerifier.sol drifted from the regen output. Run tools/zk/regen_amoe_plonk_verifier.sh."
  fi
