# `amoe/tools/zk/` — AMOE zero-knowledge tooling
...
- `contracts/lottery/zk/AmoePlonkVerifier.sol` (header banner +
  patched body)
...
`contracts/lottery/zk/AmoePlonkVerifier.sol`. Inspect the diff
...
`amoe/tools/ci/check_amoe_plonk_patch.sh` runs on every PR/push touching the
...
|---|---|---|
| "still contains 'contract PlonkVerifier'" | Someone re-exported the verifier with stock snarkjs | Run `amoe/tools/zk/regen_amoe_plonk_verifier.sh` |
...
| "drifted from the regen output" | Hand-edits diverged from the regenerator | Either regenerate with the script, or update `amoe/tools/zk/_patch_amoe_plonk_verifier.py` to reflect the new canonical form |
