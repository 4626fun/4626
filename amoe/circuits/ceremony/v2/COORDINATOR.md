...
- [ ] Tagged the manifest commit:
      ...
      ```
- [ ] Confirmed `amoe/circuits/build/pot14_final.ptau` matches the SHA-256
...
3. Run the verification script:
   ...
   bash amoe/circuits/ceremony/v2/scripts/01-verify-contribution.sh \
       amoe/circuits/build/amoe_v2_<N>.zkey
   ```
   This script:
...
```bash
bash amoe/circuits/ceremony/v2/scripts/02-finalize.sh \
...
- [ ] Commit the final ceremony artifacts:
      - `amoe/circuits/build/amoe_v2_final.zkey` — large, may want git-lfs
        or a release attachment instead.
      - `amoe/circuits/build/verification_key_v2.json` — small, commit.
      - `amoe/circuits/build/AmoeGroth16Verifier_v2.sol` — commit.
      - `amoe/circuits/ceremony/v2/ceremony_transcript_v2.txt` — commit.
- [ ] Update `amoe/circuits/CEREMONY.md` §"v2 contributor table" with the
