...
declarations in `circuits/amoe/amoe_eligibility.circom` exactly, so any
...
amoe-prover emit-verifier \
  --vk   circuits/amoe/build/verification_key.json \
...
generate it with `tools/zk/regen_amoe_fixture.sh`.
