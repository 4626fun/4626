...
contract OVaultAdminModule is OVaultModuleBase, IOVaultModuleIdentity {
    ...
    /// @dev Gap-analysis G-2: delay *decreases* are scheduled under the current delay.
    uint8 internal constant RISK_KIND_RISK_CONFIG_DELAY = 5;
    ...
    function setRiskConfigDelay(uint64 delay) external onlyDelegateCall {
        // Gap-analysis G-2: no post-init zero — the constructor seeds MIN_RISK_CONFIG_DELAY,
        // and a reachable 0 would reopen the instant-fee path the delay exists to prevent.
        if (delay < MIN_RISK_CONFIG_DELAY || delay > MAX_RISK_CONFIG_DELAY) {
            revert RiskConfigDelayOutOfBounds(delay, MIN_RISK_CONFIG_DELAY, MAX_RISK_CONFIG_DELAY);
        }
        // Increases apply immediately; decreases schedule under the *current* delay.
        if (delay >= riskConfigDelay) {
            riskConfigDelay = delay;
            emit UpdateRiskConfigDelay(delay);
            return;
        }
        _scheduleRiskChange(RISK_KIND_RISK_CONFIG_DELAY, address(0), delay);
    }
    ...
    function _scheduleRiskChange(uint8 kind, address target, uint256 value) internal {
        ...
        // Gap-analysis G-3: fee/cap decreases execute immediately.
        // Still conflict with any pending risk config — otherwise management can
        // schedule an increase, instantly cut the live fee/cap for optics, then
        // execute the original pending increase with arbitrarily little notice
        // after the cut (bypassing riskConfigDelay depositor notice).
        if (_isRiskReducing(kind, target, value)) {
    ...
    function _isRiskReducing(uint8 kind, address target, uint256 value) internal view returns (bool) {
        if (kind == RISK_KIND_PERFORMANCE_FEE) return value < performanceFee;
        if (kind == RISK_KIND_MANAGEMENT_FEE) return value < managementFee;
        if (kind == RISK_KIND_STRATEGY_MAX_ASSETS) return value < strategyMaxAssets[target];
        return false;
    }
    ...
    function _executeRiskChange(uint8 kind, address target, uint256 value) internal {
        ...
        if (kind == RISK_KIND_RISK_CONFIG_DELAY) {
            uint64 d = uint64(value);
            if (d < MIN_RISK_CONFIG_DELAY || d > MAX_RISK_CONFIG_DELAY) {
                revert RiskConfigDelayOutOfBounds(d, MIN_RISK_CONFIG_DELAY, MAX_RISK_CONFIG_DELAY);
            }
            riskConfigDelay = d;
            emit UpdateRiskConfigDelay(d);
            return;
        }
    ...
    /// @notice Asset-denominated deposit ceiling. 0 == uncapped (Yearn-style).
    function setDepositLimit(uint256 _depositLimit) external onlyDelegateCall {
        depositLimit = _depositLimit;
        emit DepositLimitUpdated(_depositLimit);
    }

    /// @notice Default maxLoss (bps) for 3-arg withdraw/redeem. 0 == 10_000 (allow all).
    function setDefaultMaxLossBps(uint16 _defaultMaxLossBps) external onlyDelegateCall {
        if (_defaultMaxLossBps > MAX_BPS) revert InvalidAmount();
        defaultMaxLossBps = _defaultMaxLossBps;
        emit DefaultMaxLossBpsUpdated(_defaultMaxLossBps);
    }

    /// @notice Gap-analysis G-4: optional NAV envelope for keeper tend/rebalance. 0 == off.
    function setMaxRebalanceDeviationBps(uint16 _maxRebalanceDeviationBps) external onlyDelegateCall {
        if (_maxRebalanceDeviationBps > MAX_BPS) revert InvalidAmount();
        maxRebalanceDeviationBps = _maxRebalanceDeviationBps;
        emit MaxRebalanceDeviationBpsUpdated(_maxRebalanceDeviationBps);
    }
