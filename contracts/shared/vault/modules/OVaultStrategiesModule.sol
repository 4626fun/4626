...
contract OVaultStrategiesModule is OVaultModuleBase, IOVaultModuleIdentity {
    ...
    function reinstateImpairedStrategy(address strategy, uint256 epochId) external onlyDelegateCall {
        ...
        // Gap-analysis G-1: reinstating re-admits the strategy's residual value to NAV,
        // but the post-finalize report() already booked the write-down (baseline reset
        // to NAV ex-strategy). Without a resync the next report() books the recovery as
        // fresh profit and mints performance-fee shares on recovered principal. Reset
        // the reference points to post-reinstate reality — same pattern as the
        // AUDIT-2026-07-01-H01 baseline reset in report().
        uint256 nav = _navIncludingStrategies();
        totalAssetsAtLastReport = nav;
        uint256 supply = _totalSupply;
        trustedPpsCheckpoint = supply == 0
            ? 1e18
            : ((nav + OVaultModuleConstants.VIRTUAL_ASSETS_UNITS) * 1e18) / (supply + OVaultModuleConstants.VIRTUAL_SHARES_UNITS);

        emit ImpairedStrategyReinstated(strategy, epochId);
    }

    /// @dev NAV exactly as the core module's report() computes it (tracked idle +
    ///      non-impaired active strategies), so baseline resyncs stay consistent
    ///      with the next report's profit/loss delta.
    function _navIncludingStrategies() internal view returns (uint256 total) {
        total = coinBalance;
        uint256 len = strategyList.length;
        for (uint256 i; i < len; i++) {
            address s = strategyList[i];
            if (activeStrategies[s] && !strategyImpaired[s]) {
                total += _getStrategyAssetsSafe(s);
            }
        }
    }
    ...
    function _getStrategyAssetsSafe(address strategy) internal view returns (uint256 assets) {
        ...
        return _getStrategyAssetsForDebtPurchase(strategy);
    }

    /// @dev Residual sizing for `buyDebt` consideration. Unlike `_getStrategyAssetsSafe`,
    ///      does not zero impaired strategies — residual still belongs to the buyer.
    function _getStrategyAssetsForDebtPurchase(address strategy) internal view returns (uint256 assets) {
    ...
    {
        totalWithdrawn = _withdrawFromStrategies(amountNeeded, maxLossBps, lossBasisAssets);
    }

    function _withdrawFromStrategies(uint256 amountNeeded, uint256 maxLossBps, uint256 lossBasisAssets)
        internal
        returns (uint256 totalWithdrawn)
    {
        if (maxLossBps > MAX_BPS) revert InvalidAmount();
        uint256 remaining = amountNeeded;
        uint256 totalAssessedLoss;
        // Yearn: max_loss is versus the user's full assets request. Fall back to the
        // strategy deficit only if a caller omitted the basis (should not happen on
        // the core withdraw/redeem path).
    ...
    /// @notice Yearn-style absolute debt targeting for one strategy.
    /// @dev Pulls excess debt to idle or deploys idle up to `targetDebt`, respecting
    ///      `minimumTotalIdle` / `deploymentThreshold` and optional `strategyMaxDebt`.
    function updateDebt(address strategy, uint256 targetDebt) external onlyDelegateCall returns (uint256 newDebt) {
        ...
        uint256 maxDebt = strategyMaxDebt[strategy];
        if (maxDebt != 0 && targetDebt > maxDebt) revert InvalidAmount();

        uint256 currentDebt = strategyDebt[strategy];
        if (targetDebt == currentDebt) {
            emit DebtTargetUpdated(strategy, currentDebt, targetDebt, currentDebt);
            return currentDebt;
        }

        if (targetDebt < currentDebt) {
            uint256 toPull = currentDebt - targetDebt;
            // Yearn update_debt: refuse debt decreases while the strategy has unrealised
            // losses. Pulling NAV < debt without writing down the loss leaves phantom
            // strategyDebt/totalDebt (and inflates totalAssets if getTotalAssets reverts).
            uint256 unrealizedLoss = _assessUnrealisedLoss(strategy, currentDebt, toPull);
            if (unrealizedLoss > 0) revert StrategyHasUnrealisedLosses(strategy, unrealizedLoss);

            uint256 withdrawn = _tryWithdrawFromStrategyMeasured(strategy, toPull);
            uint256 debtReduction = withdrawn > currentDebt ? currentDebt : withdrawn;
            ...
            emit DebtUpdated(strategy, currentDebt, newDebt);
            emit StrategyWithdrawn(strategy, withdrawn);
            emit DebtTargetUpdated(strategy, currentDebt, targetDebt, newDebt);
            return newDebt;
        }

        uint256 toDeploy = targetDebt - currentDebt;
        uint256 idleBalance = _syncCoinBalance();
        uint256 minIdle = minimumTotalIdle > deploymentThreshold ? minimumTotalIdle : deploymentThreshold;
        uint256 deployable = idleBalance > minIdle ? idleBalance - minIdle : 0;
        if (toDeploy > deployable) toDeploy = deployable;
        if (toDeploy == 0) {
            emit DebtTargetUpdated(strategy, currentDebt, targetDebt, currentDebt);
            return currentDebt;
        }

        uint256 deposited = _depositIntoStrategyMeasured(strategy, toDeploy);
        newDebt = currentDebt + deposited;
        strategyDebt[strategy] = newDebt;
        totalDebt += deposited;
        emit DebtUpdated(strategy, currentDebt, newDebt);
        emit DebtTargetUpdated(strategy, currentDebt, targetDebt, newDebt);
    ...
    function setStrategyMaxDebt(address strategy, uint256 maxDebt) external onlyDelegateCall {
        if (strategy == address(0)) revert ZeroAddress();
        strategyMaxDebt[strategy] = maxDebt;
        emit StrategyMaxDebtUpdated(strategy, maxDebt);
    }

    function tend() external onlyDelegateCall {
        if (vaultMode != VaultMode.Normal) revert VaultNotNormal();
        uint256 navBefore = _navIncludingStrategies();
        uint256 idleBalance = _syncCoinBalance();
        if (idleBalance > deploymentThreshold && totalStrategyWeight > 0) {
            _deployToStrategies();
        }
        _enforceRebalanceDeviation(navBefore);
    }
    ...
    /// @dev Gap-analysis G-4: when enabled, tend/rebalance may not move NAV beyond maxBps.
    function _enforceRebalanceDeviation(uint256 navBefore) internal view {
        uint16 maxBps = maxRebalanceDeviationBps;
        if (maxBps == 0) return;
        uint256 navAfter = _navIncludingStrategies();
        if (navBefore == 0) {
            if (navAfter > 0) revert RebalanceDeviationExceeded(navBefore, navAfter, maxBps);
            return;
        }
        uint256 delta = navAfter > navBefore ? navAfter - navBefore : navBefore - navAfter;
        if ((delta * MAX_BPS) / navBefore > maxBps) {
            revert RebalanceDeviationExceeded(navBefore, navAfter, maxBps);
        }
    }
    ...
    /// @notice Yearn-style debt purchase: buyer pays assets, debt book is reduced, and
    ///         withdrawable strategy residual is transferred to the buyer as consideration.
    /// @dev Impaired epochs route the buyer payment into recovery escrow (claim holders)
    ///      while still attempting to deliver strategy residual to the buyer.
    function buyDebt(address strategy, uint256 amount) external onlyDelegateCall {
        if (!activeStrategies[strategy] && !strategyImpaired[strategy]) revert StrategyNotActive();

        uint256 currentDebt = strategyDebt[strategy];
        if (currentDebt == 0 || amount == 0) revert NothingToBuy();

        uint256 purchaseAmount = amount > currentDebt ? currentDebt : amount;

        // Snapshot residual BEFORE debt reduction. Use the debt-purchase reader (not
        // `_getStrategyAssetsSafe`) so impaired legs still size consideration — the safe
        // reader hard-zeros impaired strategies for NAV, which would skip residual delivery.
        uint256 strategyAssets = _getStrategyAssetsForDebtPurchase(strategy);
        uint256 toWithdraw = purchaseAmount > strategyAssets ? strategyAssets : purchaseAmount;

        IERC20 coin = _vaultAsset();
        uint256 beforeBal = coin.balanceOf(address(this));
        coin.safeTransferFrom(msg.sender, address(this), purchaseAmount);
        uint256 afterBal = coin.balanceOf(address(this));
        uint256 received = afterBal - beforeBal;
        if (received != purchaseAmount) revert TransferAmountMismatch(purchaseAmount, received);
        coinBalance = afterBal;

        uint256 newDebt = currentDebt - purchaseAmount;
        strategyDebt[strategy] = newDebt;
        totalDebt -= purchaseAmount;

        // Consideration: pull residual strategy assets (may be < purchaseAmount under loss).
        uint256 withdrawn;
        if (toWithdraw > 0) {
            withdrawn = _tryWithdrawFromStrategyMeasured(strategy, toWithdraw);
        }
        ...
        if (strategyImpaired[strategy] && impairmentRecoveryEscrow != address(0)) {
            ...
            if (epochId != 0) {
                ImpairmentEpoch storage epoch = impairmentEpochs[epochId];
                // Mirror eject / notifyImpairmentRecovery claim-book rules so escrow notify
                // cannot leave totalRecovered=0 (unclaimable recovery).
                bool claimSurfaceLive = epoch.status == ImpairmentEpochStatus.Finalized
                    || epoch.status == ImpairmentEpochStatus.Resolved;
                if (claimSurfaceLive) {
                    if (epoch.recoveryAsset == address(0)) {
                    ...
                    if (epoch.recoveryAsset == address(coin)) {
                        coin.safeTransfer(impairmentRecoveryEscrow, purchaseAmount);
                        IOVaultRecoveryEscrowStrategyModule(impairmentRecoveryEscrow).notifyRecovery(
                            address(coin), epochId, purchaseAmount
                        );
                        epoch.totalRecovered += purchaseAmount;
                        // Do NOT decrease totalAssetsAtLastReport here. Unlike
                        // notifyImpairmentRecovery (ODA-427-F7), the buyer payment was never
                        // part of lasting vault NAV — it is transferFrom'd in and immediately
                        // escrowed. Decreasing the report baseline invents phantom profit equal
                        // to purchaseAmount on the next report() (fee mint / depositor dilution)
                        // whenever healthy idle or other strategies remain.
                        emit ImpairmentRecoveryNotified(epochId, address(coin), purchaseAmount);
                        coinBalance = coin.balanceOf(address(this));
                    } else {
                        emit ImpairmentRecoveryAssetMismatch(
                            epochId, epoch.recoveryAsset, address(coin), purchaseAmount
                        );
                    }
                }
            }
        ...
        if (withdrawn > 0) {
            uint256 pushBefore = coin.balanceOf(address(this));
            coin.safeTransfer(msg.sender, withdrawn);
            uint256 pushAfter = coin.balanceOf(address(this));
            uint256 spent = pushBefore - pushAfter;
            if (spent != withdrawn) revert TransferAmountMismatch(withdrawn, spent);
            coinBalance = pushAfter;
        }

        emit DebtUpdated(strategy, currentDebt, newDebt);
        emit DebtPurchased(strategy, purchaseAmount, msg.sender);
    }
