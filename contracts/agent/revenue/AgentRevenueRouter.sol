...
contract AgentRevenueRouter is Ownable, ReentrancyGuard {
    ...
    struct BatchAction {
        // kind=0 => convertAndQueue (v3 path / direct agent token)
    ...
    constructor(
        ...
        if (
            _agentToken == address(0) || _vault == address(0) || _burnStream == address(0) || _shareOFT == address(0)
    ...
    function setSwapPath(address tokenIn, bytes calldata path) external onlyOwner {
        ...
        if (tokenIn == address(agentToken) || tokenIn == address(shareOFT)) revert InvalidPath(tokenIn);
