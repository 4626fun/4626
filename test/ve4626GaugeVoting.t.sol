...
contract ve4626GaugeVotingTest is Test {
    ...
    function testVaultProbabilityBoost_NoVotesReturnsZero() public {
        ...
        assertEq(voting.getVaultProbabilityBoostPPM(vault1), 0);
        assertEq(voting.getVaultProbabilityBoostPPM(vault2), 0);
        assertEq(voting.getVaultProbabilityBoostPPM(vault3), 0);
    }

    function testVaultProbabilityBoost_WithVotesProportional() public {
        // Ensure 5 whitelisted vaults for clean math
        voting.setVaultWhitelist(makeAddr("vault4"), true);
        voting.setVaultWhitelist(makeAddr("vault5"), true);
        ...
        assertEq(voting.getVaultProbabilityBoostPPM(vault1), 35_000);
        assertEq(voting.getVaultProbabilityBoostPPM(vault2), 0);
        assertEq(voting.getVaultProbabilityBoostPPM(vault3), 0);
    }

    function testVaultProbabilityBoost_TwoVaultSplitUsesFullBoundedBudget() public {
        _lockTokens(alice, 100 ether, FOUR_YEARS);
        vm.warp(voting.genesisEpochStart() + 1);
        ...
        uint256 first = voting.getVaultProbabilityBoostPPM(vault1);
        uint256 second = voting.getVaultProbabilityBoostPPM(vault2);
    ...
    function testVaultProbabilityBoost_NoVotesSingleVaultStillZero() public {
    ...
    // ================================
    // EMERGENCY RESET (surface-safe)
    // ================================

    function testMidEpochDelist_BurnsBoostBudgetShare_DoesNotRedistribute() public {
        // Two vaults, equal votes → each gets half of 69_420 PPM.
        // Delist vault2 mid-epoch: vault2 boost=0; vault1 stays at half (budget burned, not reallocated).
        uint256 genesis = voting.genesisEpochStart();
        vm.warp(genesis - WEEK);
        _lockTokens(alice, 100 ether, FOUR_YEARS);
        _lockTokens(bob, 100 ether, FOUR_YEARS);
        ...
        uint256 halfBudget = 69_420 / 2;
        uint256 boost1 = voting.getVaultProbabilityBoostPPM(vault1);
        uint256 boost2 = voting.getVaultProbabilityBoostPPM(vault2);
        assertEq(boost1, halfBudget);
        assertEq(boost2, halfBudget);
        assertEq(voting.getTotalWeight(), voting.getVaultWeight(vault1) + voting.getVaultWeight(vault2));

        // Mid-epoch delist vault2 (whitelist path; surface mode off)
        voting.setVaultWhitelist(vault2, false);
        assertFalse(voting.canReceiveVotes(vault2));
        assertTrue(voting.canReceiveVotes(vault1));

        assertEq(voting.getVaultProbabilityBoostPPM(vault2), 0);
        // Denominator still includes vault2 weight → vault1 does not jump to full budget
        assertEq(voting.getVaultProbabilityBoostPPM(vault1), halfBudget);
        assertGt(voting.getVaultWeight(vault2), 0); // weight frozen until re-vote / epoch end
    }

    function testEmergencyReset_ZerosVotedVaults_EvenWhenNotOnWhitelist() public {
        // Surface-only vault: eligible via registry, never entered _whitelistedVaults.
        GaugeSurfaceRegistry4626 surfaces = new GaugeSurfaceRegistry4626(owner);
        address surfaceOnly = makeAddr("surfaceOnlyVault");
        surfaces.registerSurface(
            surfaceOnly, IRegistry4626.VaultKind.Creator, keccak256("creator"), true, true, true
        );
        ...
        _vote(alice, surfaceOnly, 100);
        uint256 epoch = voting.currentEpoch();
        uint256 weightBefore = voting.getVaultWeightAtEpoch(epoch, surfaceOnly);
        assertGt(weightBefore, 0);
        assertEq(voting.getTotalWeight(), weightBefore);

        // surfaceOnly is not on the local whitelist set
        assertFalse(voting.isWhitelistedVault(surfaceOnly));

        voting.emergencyResetAllVotes();

        assertEq(voting.getVaultWeightAtEpoch(epoch, surfaceOnly), 0);
        assertEq(voting.getTotalWeight(), 0);
        // Stale user weight must read as 0 until re-vote (generation guard)
        assertEq(voting.getUserVoteWeightAtEpoch(epoch, alice, surfaceOnly), 0);

        // Re-vote rebuilds clean aggregates (no phantom pre-reset weight)
        _vote(alice, surfaceOnly, 100);
