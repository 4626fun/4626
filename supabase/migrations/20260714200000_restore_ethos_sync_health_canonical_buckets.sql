-- Restore canonical Ethos sync health buckets (migration 046 semantics).
-- Looker Studio + /api/v1/chat/ethos-sync* read v_zora_owner_ethos_sync_health.
--
-- Bucket definitions (only for rows where ethos_score IS NULL):
--   rows_missing_no_identity_key  — no address: identity key in user_ethos_identity_keys
--   rows_missing_no_matched_cache — identity key exists, no matched ethos_userkey_scores row
--   rows_missing_projection_gap   — matched cache exists, projection still missing score

BEGIN;

DROP VIEW IF EXISTS public.v_zora_owner_ethos_sync_health;

CREATE VIEW public.v_zora_owner_ethos_sync_health
...
COMMENT ON VIEW public.v_zora_owner_ethos_sync_health IS
  'Single-row Ethos sync health for Looker + ethos-sync API. Missing-score buckets partition NULL ethos_score rows by identity key, matched cache, and projection lag.';

GRANT SELECT ON public.v_zora_owner_ethos_sync_health TO authenticated;
GRANT SELECT ON public.v_zora_owner_ethos_sync_health TO service_role;
