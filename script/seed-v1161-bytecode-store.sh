...
# Idempotent re-seed of UniversalBytecodeStoreV2 for the v1.16.1 bytecode epoch
# (CCALaunchArm rename + post-CCA share-mesh LP manager bytecode).
#
# Prerequisites:
...
#   SEED_OFFSET=0 SEED_LIMIT=10 ./script/seed-v1161-bytecode-store.sh
#   SEED_OFFSET=10 SEED_LIMIT=10 ./script/seed-v1161-bytecode-store.sh
...
if [[ ! -f "$ROOT_DIR/deployments/base/v1.16.1-bytecode-manifest.json" ]]; then
  echo "Missing manifest. Run: ./script/generate_bytecode_manifest.sh v1.16.1" >&2
  exit 1
fi

echo "Seeding UniversalBytecodeStoreV2 at ${UNIVERSAL_BYTECODE_STORE}"
echo "  Manifest: v1.16.1"
