 ...
 *         - `AmoeGroth16Verifier`: emitted from `circuits/amoe/build/amoe_final.zkey`
 *           after a multi-contributor phase-2 ceremony (see CEREMONY.md).
 *         - `LotteryAmoeRouter`: holds the per-epoch allowlist Merkle root,
 *           verifies proofs, and forwards entries to the lottery manager.
 *
 * @dev    Required env vars:
 *         - PRIVATE_KEY:           deployer
 *         - AMOE_OWNER:            router owner (rotates verifier, sets consumer)
 *         - AMOE_PUBLISHER:        backend signer that publishes daily Merkle roots
 *
...
contract DeployLotteryAmoeRouter is Script {
    function run() external returns (AmoeGroth16Verifier verifier, LotteryAmoeRouter router) {
        ...
        console.log("AmoeGroth16Verifier:", address(verifier));
