...
export async function executeSolanaLotteryConfirm(payload: Record<string, unknown> = {}): Promise<Record<string, unknown>> {
  ...
  const response = await fetch(`${baseUrl()}/keeper/solana/lottery-confirm`, {
  ...
  if (!response.ok || json.success === false) throw new Error(`${SOLANA_LOTTERY_LZ_TRANSPORT_UNAVAILABLE}:keeper_confirm_${response.status}:${String(json.error ?? text).slice(0, 200)}`)
