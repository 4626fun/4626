...
describe('canonical sender context plumbing', () => {
  ...
  it('preserves the protected automation block from the CRE active-vault feed', async () => {
    ...
    const { fetchActiveVaults } = await import('../kpr-workflows/_shared/strategyQueue.js')
  ...
  it(
    ...
    } = await import('../kpr-workflows/_shared/ajnaManager.js')
  ...
  it('surfaces enabled-but-unusable Ajna automation as a canonical sender error', async () => {
    ...
    const { evaluateAndEnqueueAjnaActions } = await import('../kpr-workflows/_shared/ajnaManager.js')
