#!/usr/bin/env node
/**
 * Walks every kpr/kpr-workflows/<workflow>/config.{staging,production}.json
 * and refuses any top-level key that starts with `mock`.
 *
 * Run from the repo root:
 *   node kpr/scripts/check-no-mock-in-non-local-config.mjs
 *
 * Exit codes:
 *   0 — all configs clean
...
const ROOT = fileURLToPath(new URL("../kpr-workflows/", import.meta.url))
