...
function normalizeVerdict(value2) {
  const raw = toText(value2).toLowerCase();
  if (raw === "pass" || raw === "watch" || raw === "critical" || raw === "unknown") {
    return raw;
  }
  return null;
}
function defaultSuggestedAction(verdict) {
  if (verdict === "critical")
    return "Pause keeper-triggered writes and investigate immediately.";
  if (verdict === "watch")
    return "Review warnings and monitor closely on the next run.";
  if (verdict === "pass")
    return "No action required; continue normal monitoring cadence.";
  return "Investigate telemetry and rerun checks.";
}
function deriveDeterministicVerdict(alerts) {
  if (alerts.some((alert) => alert.severity === "critical"))
    return "critical";
  if (alerts.some((alert) => alert.severity === "warning" || alert.severity === "info"))
    return "watch";
  return "pass";
}
function createAiFallbackResult(alerts, error) {
  const verdict = deriveDeterministicVerdict(alerts);
  return {
    enabled: false,
    verdict,
    confidence: null,
    summary: alerts.length > 0 ? `AI assessment unavailable; using deterministic checks with ${alerts.length} alert(s).` : "AI assessment unavailable; deterministic checks indicate no active alerts.",
    suggestedAction: defaultSuggestedAction(verdict),
    ...error ? { error } : {}
  };
}
function normalizeAiResult(raw, alerts) {
  ...
  const suggestedAction = truncate(toText(source.suggestedAction) || defaultSuggestedAction(verdict), MAX_ACTION_LENGTH);
  return {
...
var onCronTrigger = (runtime2) => {
  ...
  let checksRun = 0;
  try {
    const data = evmRead(runtime2, evmClient, coinAddr, CreatorCoinABI, "payoutRecipient");
    const payoutRecipient = decodeAddress2(CreatorCoinABI, "payoutRecipient", data).toLowerCase();
    checksRun++;
    if (payoutRecipient !== gaugeAddr.toLowerCase()) {
      ...
      runtime2.log("Check 1 PASS: payoutRecipient matches GaugeController");
    }
  } catch {
    runtime2.log("Check 1 ERROR: failed to read payoutRecipient");
  }
  try {
    const burnData = evmRead(runtime2, evmClient, gaugeAddr, GaugeControllerABI, "burnShareBps");
    const lotteryData = evmRead(runtime2, evmClient, gaugeAddr, GaugeControllerABI, "lotteryShareBps");
    const creatorData = evmRead(runtime2, evmClient, gaugeAddr, GaugeControllerABI, "creatorShareBps");
    const protocolData = evmRead(runtime2, evmClient, gaugeAddr, GaugeControllerABI, "protocolShareBps");
    ...
    if (totalBps !== 1e4) {
    ...
    } else if (burnBps !== cfg.expectedBurnShareBps || lotteryBps !== cfg.expectedLotteryShareBps || creatorBps !== cfg.expectedCreatorShareBps || protocolBps !== cfg.expectedProtocolShareBps) {
      pendingAlerts.push({
        vaultAddress: vaultAddr,
      ...
      runtime2.log("Check 2 PASS: BPS config correct");
    }
  } catch {
    runtime2.log("Check 2 ERROR: failed to read BPS config");
  }
  try {
    const data = evmRead(runtime2, evmClient, gaugeAddr, GaugeControllerABI, "vault");
    const gaugeVault = decodeAddress2(GaugeControllerABI, "vault", data).toLowerCase();
    checksRun++;
    if (gaugeVault !== vaultAddr.toLowerCase()) {
      ...
      runtime2.log("Check 3 PASS: GaugeController vault wiring correct");
    }
  } catch {
    runtime2.log("Check 3 ERROR: failed to read GaugeController vault()");
  }
  if (burnStreamAddr) {
    try {
      const activeData = evmRead(runtime2, evmClient, burnStreamAddr, BurnStreamABI, "activeShares");
      const burnedData = evmRead(runtime2, evmClient, burnStreamAddr, BurnStreamABI, "burnedActive");
      const epochStartData = evmRead(runtime2, evmClient, burnStreamAddr, BurnStreamABI, "activeEpochStart");
      ...
      const staleThreshold = runtime2.config.staleThresholdSeconds;
      if (activeShares > 0n && activeEpochStart > 0 && nowSeconds - activeEpochStart > staleThreshold) {
        ...
        runtime2.log("Check 4 PASS: Burn stream healthy");
      }
    } catch {
      runtime2.log("Check 4 ERROR: failed to read burn stream state");
    }
  } else {
    runtime2.log("Check 4 SKIP: No burn stream configured");
  }
  try {
    const balData = evmRead(runtime2, evmClient, vaultAddr, ERC20ABI, "balanceOf", [gaugeAddr]);
    const gaugeBalance = decodeBigInt(ERC20ABI, "balanceOf", balData);
    const distData = evmRead(runtime2, evmClient, gaugeAddr, GaugeControllerABI, "lastDistribution");
    ...
    if (gaugeBalance > 0n && lastDistribution > 0 && nowSeconds - lastDistribution > staleThreshold) {
      ...
      runtime2.log("Check 5 PASS: GaugeController balance/distribution OK");
    }
  } catch {
  ...
  if (pendingAlerts.length > 0) {
    ...
    for (const alert of pendingAlerts) {
      const sent = runtime2.runInNodeMode((nr) => sendAlert(nr, httpClient, apiKey, alert), consensusIdenticalAggregation())().result();
      if (sent)
        alertsSent++;
    }
  } else {
    runtime2.log("All checks passed — no alerts");
  }
  return {
    vaultAddress: vaultAddr,
    checksRun,
