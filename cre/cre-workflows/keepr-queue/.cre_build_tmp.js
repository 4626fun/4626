...
var onCronTrigger = (runtime2) => {
  ...
  runtime2.log("Keepr queue executor starting");
  const httpClient = new ClientCapability;
  const result = runtime2.runInNodeMode((nodeRuntime) => fetchPendingActions(nodeRuntime, httpClient, apiKey), consensusIdenticalAggregation())().result();
  runtime2.log(`Queue processing complete: processed=${result.processed} succeeded=${result.succeeded} failed=${result.failed} retried=${result.retried} skipped=${result.skipped}`);
  return result;
};
